# FinOps - Optimización de Costes Cloud

**Fecha de creación:** 2026-01-09 22:39  
**Última actualización:** 2026-01-13 12:55  
**Versión:** 2.0.0  
**Categoría:** Implementación

---

## 📑 Tabla de Contenidos (TOC)

1. [Propósito](#1-propósito)
2. [Principios FinOps](#2-principios-finops)
3. [Modelo de Costes](#3-modelo-de-costes)
4. [Estrategias de Optimización](#4-estrategias-de-optimización)
5. [Automatización](#5-automatización)
6. [Dashboards y Reporting](#6-dashboards-y-reporting)
7. [Gobernanza de Costes](#7-gobernanza-de-costes)
8. [Registro de Cambios](#8-registro-de-cambios)

---

## 1. Propósito

FinOps (Financial Operations) permite:

- **Visibilidad** total de costes cloud
- **Responsabilidad** distribuida por equipo/tenant
- **Optimización** continua de recursos
- **Predicción** de costes futuros
- **Alineación** negocio-tecnología

---

## 2. Principios FinOps

### 2.1 Fases del Ciclo FinOps

```mermaid
graph LR
    A[INFORM] --> B[OPTIMIZE]
    B --> C[OPERATE]
    C --> A
    
    style A fill:#4CAF50
    style B fill:#2196F3
    style C fill:#FF9800
```

| Fase | Objetivo | Actividades |
|------|----------|-------------|
| **INFORM** | Visibilidad | Dashboards, etiquetado, alertas |
| **OPTIMIZE** | Eficiencia | Right-sizing, reservas, spot |
| **OPERATE** | Gobernanza | Políticas, budgets, revisiones |

### 2.2 Responsabilidades

| Rol | Responsabilidad |
|-----|-----------------|
| **Engineering** | Optimizar código y recursos |
| **Finance** | Budgets y forecasting |
| **Business** | Unit economics y ROI |
| **FinOps Lead** | Coordinar y educar |

---

## 3. Modelo de Costes

### 3.1 Estructura de Costes SaaS

```
                    COSTE TOTAL
                         │
        ┌────────────────┼────────────────┐
        │                │                │
        ▼                ▼                ▼
    ┌───────┐       ┌───────┐       ┌───────┐
    │ INFRA │       │  SaaS │       │ PEOPLE│
    │  40%  │       │  35%  │       │  25%  │
    └───┬───┘       └───┬───┘       └───────┘
        │               │
   ┌────┼────┐     ┌────┼────┐
   │    │    │     │    │    │
  VM   DB  CDN  Stripe Ecwid 3rd-party
```

### 3.2 Costes por Categoría

| Categoría | Servicio | Coste Mensual Est. | Notas |
|-----------|----------|-------------------|-------|
| **Compute** | VPS/Container | €200-500 | Variable por tráfico |
| **Database** | MariaDB Managed | €100-300 | Por réplicas |
| **Cache** | Redis | €50-100 | Por memoria |
| **Storage** | Object Storage | €20-50 | Por GB |
| **CDN** | Cloudflare | €0-100 | Tier free disponible |
| **Backup** | Offsite | €30-50 | Por retención |
| **Monitoring** | Grafana Cloud | €0-50 | Tier free disponible |
| **Email** | SMTP Transactional | €20-100 | Por volumen |
| **Stripe** | Procesamiento | 1.4-2.9% + €0.25 | Por transacción |
| **Ecwid** | E-commerce | €0-35/tienda | Por plan tenant |

### 3.3 Coste por Tenant

```yaml
# Fórmula de coste por tenant
tenant_cost:
  fixed:
    - platform_share: €2/mes  # Infraestructura compartida
    
  variable:
    - storage: €0.05/GB/mes
    - bandwidth: €0.01/GB
    - api_calls: €0.001/1000 calls
    - email_sends: €0.01/email
    
  passthrough:
    - stripe_fees: 1.4% + €0.25/tx
    - ecwid_subscription: según plan tenant
```

---

## 4. Estrategias de Optimización

### 4.1 Right-Sizing

| Recurso | Métrica | Acción si < 30% | Acción si > 80% |
|---------|---------|-----------------|-----------------|
| CPU | avg_usage | Reducir cores | Aumentar cores |
| Memoria | avg_used | Reducir RAM | Aumentar RAM |
| Disco | avg_iops | Cambiar a HDD | Cambiar a SSD |
| DB Connections | pool_usage | Reducir pool | Aumentar pool |

```promql
# Query para identificar recursos infrautilizados
avg by (instance) (rate(container_cpu_usage_seconds_total[24h])) 
  / on(instance) container_spec_cpu_quota * 100 < 30
```

### 4.2 Instancias Reservadas vs On-Demand

| Tipo | Descuento | Compromiso | Uso Recomendado |
|------|-----------|------------|-----------------|
| **On-Demand** | 0% | Ninguno | Desarrollo, picos |
| **Reserved 1yr** | 30-40% | 1 año | Producción estable |
| **Reserved 3yr** | 50-60% | 3 años | Core infrastructure |
| **Spot/Preemptible** | 70-90% | Ninguno | Batch, CI/CD |

### 4.3 Spot Instances para Staging

```yaml
# Configuración staging con spot instances
staging:
  compute:
    type: spot
    fallback: on-demand
    interruption_handler: graceful_shutdown
    max_price: 50% of on-demand
    
  schedule:
    active_hours: "08:00-20:00 CET"
    weekend: "off"
```

### 4.4 Autoscaling Basado en Coste

```yaml
# HPA con límite de coste
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: jaraba-appserver-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: appserver
  minReplicas: 2
  maxReplicas: 10
  behavior:
    scaleDown:
      stabilizationWindowSeconds: 300
      policies:
        - type: Pods
          value: 1
          periodSeconds: 60
  metrics:
    - type: Resource
      resource:
        name: cpu
        target:
          type: Utilization
          averageUtilization: 70
```

---

## 5. Automatización

### 5.1 Apagado Automático de Staging

```yaml
# ECA Model: Apagar staging fuera de horario
id: finops_staging_shutdown
label: 'FinOps: Apagar Staging Nocturno'
events:
  - plugin: cron
    id: nightly_shutdown
    configuration:
      frequency: '0 20 * * 1-5'  # 20:00 L-V
      
actions:
  - plugin: kubernetes_scale
    id: scale_down_staging
    configuration:
      namespace: jaraba-staging
      replicas: 0
      deployments:
        - appserver
        - worker
        
  - plugin: slack_notification
    id: notify_shutdown
    configuration:
      message: '💤 Staging apagado. Ahorro estimado: €X/noche'
```

### 5.2 Alertas de Anomalías de Coste

```yaml
# Alertmanager: Anomalía de coste
groups:
  - name: finops
    rules:
      - alert: CostAnomalyDetected
        expr: |
          (cloud_cost_hourly - avg_over_time(cloud_cost_hourly[7d])) 
          / stddev_over_time(cloud_cost_hourly[7d]) > 3
        for: 1h
        labels:
          severity: warning
        annotations:
          summary: "Anomalía de coste detectada"
          description: "Coste actual {{ $value | humanize }} desviaciones sobre la media"
          
      - alert: BudgetThreshold80
        expr: cloud_cost_mtd / cloud_budget_monthly * 100 > 80
        for: 1h
        labels:
          severity: warning
        annotations:
          summary: "80% del presupuesto mensual consumido"
```

### 5.3 Limpieza Automática de Recursos

```bash
#!/bin/bash
# Script: cleanup_orphaned_resources.sh

# Eliminar snapshots antiguos (> 30 días)
aws ec2 describe-snapshots --owner-ids self \
  --query 'Snapshots[?StartTime<=`'$(date -d '30 days ago' --iso-8601)'`].SnapshotId' \
  --output text | xargs -r aws ec2 delete-snapshot --snapshot-id

# Eliminar volumes no attached
aws ec2 describe-volumes --filters "Name=status,Values=available" \
  --query 'Volumes[*].VolumeId' --output text | \
  xargs -r aws ec2 delete-volume --volume-id

# Eliminar IPs elásticas no asociadas
aws ec2 describe-addresses --query 'Addresses[?AssociationId==null].AllocationId' \
  --output text | xargs -r aws ec2 release-address --allocation-id
```

---

## 6. Dashboards y Reporting

### 6.1 Dashboard Ejecutivo

| Panel | Contenido |
|-------|-----------|
| **Coste MTD** | Gauge vs budget |
| **Forecast EOM** | Proyección fin de mes |
| **Coste por Servicio** | Pie chart |
| **Coste por Tenant** | Top 10 consumidores |
| **Tendencia Mensual** | Line chart 12 meses |
| **Ahorro Logrado** | Stat panel vs mes anterior |

### 6.2 Dashboard Técnico

| Panel | Query |
|-------|-------|
| **CPU Cost Efficiency** | `sum(rate(cpu_usage)) / sum(cpu_requested)` |
| **Memory Waste** | `sum(memory_requested - memory_used)` |
| **Idle Resources** | `count(avg_utilization < 10%)` |
| **Spot Savings** | `sum(spot_discount_amount)` |

### 6.3 Reporte Mensual

```markdown
# Reporte FinOps - [Mes Año]

## Resumen
- **Coste Total:** €X,XXX
- **vs Budget:** +X% / -X%
- **vs Mes Anterior:** +X% / -X%

## Costes por Categoría
| Categoría | Coste | % Total | Δ Mes Ant. |
|-----------|-------|---------|------------|
| Compute | €XXX | XX% | +X% |
| Database | €XXX | XX% | -X% |
| ... | ... | ... | ... |

## Top 5 Tenants por Consumo
1. Tenant A - €XXX
2. Tenant B - €XXX
...

## Optimizaciones Aplicadas
- Right-sizing: -€XXX
- Spot instances: -€XXX
- Apagado staging: -€XXX

## Recomendaciones
1. Considerar reserved instances para DB
2. Revisar tenant X por alto consumo
```

---

## 7. Gobernanza de Costes

### 7.1 Política de Etiquetado

Todos los recursos deben tener:

| Tag | Obligatorio | Ejemplo |
|-----|-------------|---------|
| `Environment` | ✅ | production, staging, dev |
| `Service` | ✅ | appserver, database, cache |
| `Team` | ✅ | platform, product |
| `CostCenter` | ✅ | CC-001 |
| `Tenant` | Si aplica | tenant-42 |

```yaml
# Terraform: Enforce tags
resource "aws_instance" "example" {
  # ...
  
  tags = {
    Environment = var.environment
    Service     = "appserver"
    Team        = "platform"
    CostCenter  = "CC-001"
    ManagedBy   = "terraform"
  }
  
  lifecycle {
    prevent_destroy = true
    ignore_changes  = [tags["LastModified"]]
  }
}
```

### 7.2 Presupuestos por Equipo

| Equipo | Budget Mensual | Alertas |
|--------|----------------|---------|
| Platform | €500 | 50%, 80%, 100% |
| Product | €300 | 50%, 80%, 100% |
| Data | €200 | 50%, 80%, 100% |
| QA | €100 | 80%, 100% |

### 7.3 Revisión Trimestral

| Actividad | Responsable | Entregable |
|-----------|-------------|------------|
| Análisis de tendencias | FinOps Lead | Report |
| Right-sizing review | Engineering | Lista de cambios |
| Reservas evaluation | Finance + FinOps | Recomendaciones |
| Budget adjustment | Finance | Nuevos budgets |

---

## 8. Registro de Cambios

| Fecha | Versión | Autor | Descripción |
|-------|---------|-------|-------------|
| 2026-01-09 | 1.0.0 | IA Asistente | Creación inicial del documento |
| 2026-01-13 | 2.0.0 | IA Asistente | Actualizado con referencia a Unit Economics (ver doc separado) |

---

## 📌 Documentos Relacionados

- [FinOps 3.0 Unit Economics](./2026-01-13_finops-unit-economics.md) - Implementación completa de LTV, CAC, Payback y Vertical Profitability
