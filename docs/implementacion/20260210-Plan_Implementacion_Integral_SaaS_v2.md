...
> **Cambios v2:** +§4.10 Seguridad AI/LLM (verificado en codebase), +§5.6 Patrón Nuevo Vertical, §6.7 ampliada con mapeo 20260118, parciales Twig actualizados (7→17), estadísticas corregidas
...
4. [Directrices Obligatorias de Implementación](#4-directrices-obligatorias-de-implementación)
   ...
   - 4.10 [Seguridad AI/LLM (Implementada)](#410-seguridad-aillm-implementada)
5. [Patrones de Implementación Estándar](#5-patrones-de-implementación-estándar)
   ...
   - 5.6 [Patrón Creación Nuevo Vertical](#56-patrón-creación-nuevo-vertical)
...
```
templates/partials/ (17 archivos verificados)
├── _header.html.twig              # Dispatcher de headers
├── _header-classic.html.twig      # Layout header clásico
├── _header-split.html.twig        # Layout header split (auth pages)
├── _header-centered.html.twig     # Layout header centrado
├── _header-minimal.html.twig      # Layout header minimal
├── _header-hero.html.twig         # Layout header con hero
├── _auth-hero.html.twig           # Hero para páginas de autenticación
├── _hero.html.twig                # Componente hero genérico
├── _footer.html.twig              # Footer del tema
├── _copilot-fab.html.twig         # FAB del copiloto IA
├── _article-card.html.twig        # Card de artículo reutilizable
├── _category-filter.html.twig     # Filtro de categorías
├── _features.html.twig            # Sección features
├── _intentions-grid.html.twig     # Grid de intenciones
├── _related-articles.html.twig    # Artículos relacionados
├── _stats.html.twig               # Componente estadísticas
└── vertical-landing-content.html.twig  # Contenido de landing vertical
```
...
### 4.10 Seguridad AI/LLM (Implementada)

> **Estado:** ✅ Implementado en el codebase actual  
> **Referencia:** Directrices §4.5, §4.6 — [Auditoría Profunda SaaS Multidimensional](../tecnicos/auditorias/20260206-Auditoria_Profunda_SaaS_Multidimensional_v1_Claude.md)

Las siguientes medidas de seguridad para endpoints AI/LLM **ya están implementadas** en el código:

| Directriz | Estado | Implementación |
|-----------|--------|----------------|
| **Rate Limiting** | ✅ | `RateLimiterService` en `ecosistema_jaraba_core` — usado por `AgentApiController`, `RagApiController`, `CopilotApiController`. Configurable en `/admin/config/system/rate-limits` |
| **Circuit Breaker LLM** | ✅ | `CopilotOrchestratorService` — threshold 5 fallos, cooldown 300s. Configurable desde `RateLimitSettingsForm` |
| **Sanitización de Prompts** | ✅ | `AIGuardrailsService` (PII redaction, injection detection) + `sanitizePromptInput()` en `JarabaRagService` + `sanitizeNliInput()` en `GroundingValidator` |
| **Aislamiento Qdrant Multi-Tenant** | ✅ | `TenantContextService` con filtros `must` obligatorios para `tenant_id`. `hasAccessToTenant()` verifica acceso |
| **Claves API en Env Vars** | ✅ | Módulo Key de Drupal + `settings.local.php` excluido de git |

**Archivos clave de seguridad AI:**
```
ecosistema_jaraba_core/src/Service/
├── RateLimiterService.php       # Rate limiting por usuario y bucket
├── AIGuardrailsService.php      # Validación y sanitización de prompts
└── SelfHealingService.php       # Circuit breaker automático

jaraba_copilot_v2/src/Service/
└── CopilotOrchestratorService.php  # Circuit breaker configurable por proveedor

jaraba_rag/src/Service/
├── TenantContextService.php     # Aislamiento Qdrant multi-tenant
├── JarabaRagService.php         # sanitizePromptInput()
└── GroundingValidator.php       # sanitizeNliInput()
```

> [!WARNING]
> **Context Window Management** (MAX_CONTEXT_TOKENS configurable) está parcialmente implementado. Actualmente trunca inputs a longitudes fijas (8000/1000 chars en GroundingValidator), pero no tiene configuración dinámica por proveedor. Pendiente de mejora.
...
### 5.6 Patrón Creación Nuevo Vertical

Proceso replicable para crear un nuevo vertical en el ecosistema:

**Paso 1 — Módulo Core**
1. Crear `jaraba_{vertical}_core/` con `*.info.yml` y `*.module`
2. Añadir `package.json` con scripts de build SCSS (obligatorio)
3. Definir Content Entities con `@ContentEntityType` annotations

**Paso 2 — Estructura SCSS**
1. Crear `scss/_variables.scss` con colores del vertical (usar tokens `var(--ej-*)`)
2. Crear `scss/main.scss` como entry point con `@use` de cada parcial
3. **NO definir** variables `$ej-*` localmente — solo consumir CSS Custom Properties del core

**Paso 3 — Frontend**
1. Crear template `page--{vertical}-*.html.twig` con Zero Region Pattern
2. Verificar parciales Twig existentes antes de crear nuevos (17 disponibles)
3. Añadir body classes en `hook_preprocess_html()` de `ecosistema_jaraba_theme.theme`
4. Registrar sugerencias en `hook_theme_suggestions_page_alter()`

**Paso 4 — Navegación Admin**
1. Registrar rutas `/admin/content/{entity}` (listado) y `/admin/structure/{entity}` (Field UI)
2. Configurar permisos en `*.permissions.yml`
3. Verificar integración con Views

**Paso 5 — Verificación**
1. Compilar SCSS: `npx sass scss/main.scss css/{vertical}.css --style=compressed`
2. Limpiar caché: `docker exec jarabasaas_appserver_1 drush cr`
3. Verificar: header visible, full-width, footer, sin sidebar, body classes aplicadas
4. Ejecutar checklist de compliance (§8)

> [!TIP]
> **Fork pattern:** El vertical más similar ya implementado sirve como base. Ejemplo: ServiciosConecta reutiliza ~70% del patrón de AgroConecta.
...
#### 6.7.1 Frontend & UX (100-103)

| Doc | Nombre | Módulo | Horas | Prioridad | Estado |
|-----|--------|--------|-------|-----------|--------|
| 100 | Frontend Architecture MultiTenant | `ecosistema_jaraba_theme` | 128-176 | 🔴 P0 | 🔶 Parcial |
...
#### 6.7.2 Admin & SEPE (104-107)

| Doc | Nombre | Módulo | Horas | Prioridad | Estado |
|-----|--------|--------|-------|-----------|--------|
| 104 | SaaS Admin Center Premium | `ecosistema_jaraba_core` | 80-100 | 🔴 P0 | 🔶 Base |
| 105-107 | SEPE Homologación | `jaraba_sepe` | 85-115 | 🔴 P0 (B2G) | ⬜ |

#### 6.7.3 Platform Services (108-117)

| Doc | Nombre | Módulo | Horas | Prioridad | Estado |
|-----|--------|--------|-------|-----------|--------|
| 108 | AI Agent Flows | `jaraba_ai_agents` | 65-85 | 🟡 P1 | ✅ Core |
...
| 115 | Security & Compliance | `ecosistema_jaraba_core` | 60-80 | 🔴 P0 | ✅ (ver §4.10) |
| 116 | Advanced Analytics | `jaraba_pixel_manager` | 95-125 | 🟡 P1 | ✅ Core |
| 117 | WhiteLabel | `ecosistema_jaraba_theme` | 80-100 | 🟢 P2 | ⬜ |

#### 6.7.4 Marca Personal (123-127)

| Doc | Nombre | Módulo | Horas | Prioridad | Estado |
|-----|--------|--------|-------|-----------|--------|
| 123-127 | Marca PepeJaraba | N/A (branding) | 40-60 | 🟢 P2 | ⬜ |

#### 6.7.5 AI Trilogy (128-130) — ✅ 100% Completada

| Doc | Nombre | Módulo | Horas | Prioridad | Estado |
|-----|--------|--------|-------|-----------|--------|
| 128 | AI Content Hub | `jaraba_content_hub` | 170-230 | 🔴 P0 | ✅ 100% |
| 129 | AI Skills System | `jaraba_ai_skills` | 145-195 | 🔴 P0 | ✅ 100% |
| 130 | Tenant Knowledge Training | `jaraba_knowledge_training` | 430-545 | 🔴 P0 | ✅ 100% |

> La AI Trilogy (Docs 128-130) fue completada al 100% en Febrero 2026.  
> Ver: [20260210-Mapeo_Especificaciones_20260118_v1.md](./20260210-Mapeo_Especificaciones_20260118_v1.md)

#### 6.7.6 Infraestructura & DevOps (131-140)

| Doc | Nombre | Área | Horas | Prioridad | Estado |
|-----|--------|------|-------|-----------|--------|
| 131 | CI/CD Pipeline | GitHub Actions | 25-35 | 🔴 P0 | 🔶 Parcial |
| 132 | Infrastructure Production | IONOS Cloud | 30-40 | 🔴 P0 | 🔶 Parcial |
| 133 | Monitoring & Observability | N/A | 25-35 | 🟡 P1 | 🔶 Parcial |
| 134 | Backup & Disaster Recovery | N/A | 20-30 | 🔴 P0 | 🔶 Parcial |
| 135 | E2E Testing Suite | Cypress | 40-50 | 🔴 P0 | ✅ Core |
| 136 | Email Templates | `jaraba_email` | 30-40 | 🟡 P1 | ✅ |
| 137 | Performance Optimization | `jaraba_performance` | 25-35 | 🟡 P1 | ✅ Core |
| 138 | CDN & Asset Delivery | N/A | 15-25 | 🟡 P1 | ⬜ |
| 139 | Database Optimization | N/A | 20-30 | 🟡 P1 | ⬜ |
| 140 | API Documentation | OpenAPI | 15-25 | 🟢 P2 | ⬜ |

#### 6.7.7 Resumen Estado Mapeo 20260118

| Bloque | Docs | Implementados | Pendientes | % |
|--------|------|---------------|------------|---|
| AI Trilogy | 128-130 | 3 | 0 | 100% |
| Frontend & UX | 100-103 | 2 | 2 | 50% |
| Platform Services | 108-117 | 4 | 6 | 40% |
| Infraestructura | 131-140 | 6 | 4 | 60% |
| Admin & SEPE | 104-107 | 0 | 2 | 0% |
| Marca Personal | 123-127 | 0 | 1 | 0% |
| **Total** | **100-140** | **15** | **15** | **50%** |
...
| Mapeo Specs 20260118 | `docs/implementacion/20260210-Mapeo_Especificaciones_20260118_v1.md` |
...
|-------|---------|-------------|
| 2026-02-10 | 1.0 | Creación inicial con 10 secciones, TOC, compliance checklist |
| 2026-02-10 | 2.0 | +§4.10 Seguridad AI/LLM verificada en codebase, +§5.6 Patrón Nuevo Vertical, §6.7 expandida con mapeo 20260118 (12/30 implementados), parciales Twig actualizados (7→17), estadísticas corregidas (55 aprendizajes) |
| 2026-02-10 | 2.1 | §6.7.6 corregida: CI/CD (Doc 131) ⬜→🔶 (5 workflows GH Actions funcionales), Backup/DR (Doc 134) ⬜→🔶 (doc 385 líneas + pre-deploy backup). +endpoint `/health` público, +`phpunit.xml`, +`.env.ci`, `deploy.yml` consolidado. Total implementados: 12→14 (47%) |
| 2026-02-10 | 2.2 | Fase 2 Hardening: Monitoring (Doc 133) ⬜→🔶 (`hook_cron` health checks + `/health` público + `verify-backups.yml` + guía UptimeRobot). Total: 14→15 (50%) |
