...
| 2026-02-16 | **41.0.0** | **Fix CI Synthetic Services Cascade (Plan Elevacion JarabaLex):** 18 Kernel tests fallaban por 4 servicios sinteticos no registrados tras commit Plan Elevacion (`jarabalex_feature_gate`, `tenant_brand_voice`, `observability`, `unified_prompt_builder`). Fix en 2 commits (3 archivos test). Nueva regla KERNEL-SYNTH-002: al anadir dependencias `@service` a `.services.yml`, actualizar TODOS los Kernel tests en el MISMO commit. Deploy a produccion verificado OK (health check + 4 componentes). 90 aprendizajes |
| 2026-02-16 | **40.0.0** | **Plan Elevacion JarabaLex v1 — 14 Fases Clase Mundial:** Nueva seccion 5.8.5 con 3 reglas especificas JarabaLex/Legal Intelligence: LEGAL-RAG-001 (disclaimer + citas verificables obligatorias en respuestas copilot legal), LEGAL-GATE-001 (FeatureGateService obligatorio en servicios con limites de plan, check()+fire()), LEGAL-BODY-001 (body classes via hook_preprocess_html(), nunca attributes.addClass() en template). 14 fases implementadas: FeatureGate, UpgradeTriggers, CopilotBridge, preprocess_html, page template zero-region, SCSS compliance, design tokens, email MJML, cross-vertical bridges, journey progression, health score, experiments, avatar navigation+funnels, QA integral. LegalCopilotAgent 6 modos con 5 metodos AgentInterface. 90 aprendizajes |
| 2026-02-16 | **39.0.0** | **Documentation Update — Zero-Region Rules + Module Registry:** Seccion 5.8.4 con 3 reglas ZERO-REGION-001/002/003 (variables via preprocess, no entity objects en render arrays, drupalSettings via preprocess). Arquitectura actualizada con 5 modulos faltantes en seccion 7.1 (jaraba_tenant_export, jaraba_dr, jaraba_legal, jaraba_privacy, ComplianceAggregatorService). Indice v55.0.0. Aprendizaje #88 zero-region pattern. |
| 2026-02-16 | **38.0.0** | **Kernel Test CI Fix Campaign — 61→0 Errors + Production Bug Fix:** 7 commits iterativos corrigieron todas las fallas de Kernel tests en CI (workflow "Jaraba SaaS - Deploy to IONOS"). **Bug de produccion corregido:** `jaraba_verifactu.module` referenciaba campo inexistente `modo_verifactu` (correcto: `is_active`) en funciones `_jaraba_verifactu_tenant_enabled()` y `_jaraba_verifactu_get_active_tenant_ids()`. **Causas raiz:** KernelTestBase no resuelve dependencias de info.yml — requiere declaracion manual de modulos de field types (options, datetime, flexible_permissions, group, file). Servicios de modulos no cargados necesitan synthetic registration + typed mocks. Entity references a entidades inexistentes se descartan silenciosamente. Timestamps `time()` flaky en CI (±1s tolerance). **Patron synthetic services establecido:** `register(ContainerBuilder)` → `setSynthetic(TRUE)` + `setUp()` → `$this->container->set('id', $this->createMock(Class::class))`. **Cleanup:** 11 stale stubs (JS/SCSS/Twig) eliminados de jaraba_legal + jaraba_dr. Library deps corregidas (drupal.dialog.ajax → drupalSettings). **Tests finales:** 1,895 Unit + 144 Kernel = 2,039 tests, 0 errores. 4 reglas nuevas: KERNEL-DEP-001, KERNEL-SYNTH-001, KERNEL-EREF-001, KERNEL-TIME-001. Aprendizaje #87 |
| 2026-02-18 | **48.0.0** | **Auditoria Integridad Planes 20260217 — Gaps medium resueltos:** 10 PHPUnit test files para 5 modulos N2 Growth Ready (~100 test methods). `declare(strict_types=1)` a 31 PHP en jaraba_servicios_conecta (37/37). CopilotApiController proactive API ComercioConecta `/api/v1/copilot/comercio/proactive`. Regla TEST-N2-001: cada modulo N2 requiere minimo 2 test files de servicio con patron mock canonico (EntityTypeManagerInterface + LoggerInterface + domain mocks). Aprendizaje #97. Arquitectura v48.0.0, Indice v64.0.0. 97 aprendizajes |
| 2026-02-18 | **47.0.0** | **Auditoria Integridad Planes 20260217 — 2 gaps criticos resueltos:** Auditoria completa de 4 planes de implementacion (AgroConecta 97%, ComercioConecta 88%→100%, ServiciosConecta 96%→100%, N2 Growth Ready 95%). **Gap 1 resuelto:** 6 MJML email templates ServiciosConecta creados en `jaraba_email/templates/mjml/serviciosconecta/` (onboarding_profesional, activacion, reengagement, upsell_starter, upsell_pro, retention) con branding #233D63 y patron canonico AgroConecta. **Gap 2 resuelto:** `declare(strict_types=1)` anadido a 177 ficheros PHP en `jaraba_comercio_conecta/src/` — compliance 100% (178/178). Verificado PB blocks ComercioConecta existentes (11/11 — falso positivo audit). Regla AUDIT-INTEGRITY-001: verificar integridad planes contra codebase antes de nueva implementacion. Aprendizaje #96. Arquitectura v47.0.0, Indice v63.0.0. 96 aprendizajes |
| 2026-02-16 | **37.0.0** | **Gap-filling Compliance Stack + ComplianceAggregatorService:** Servicios con logica de produccion completa para los 3 modulos compliance (+16,035 LOC, 97 archivos). **jaraba_dr:** 5 servicios implementados (BackupVerifier verificacion real, DrTestRunner ejecucion tests, FailoverOrchestrator orquestacion failover, IncidentCommunicator notificaciones, StatusPageManager gestion status page) + rutas API + ECA hooks + JS interactivo + 4 unit tests. **jaraba_legal:** 5 servicios implementados (TosManager, SlaCalculator, AupEnforcer, OffboardingManager, WhistleblowerChannel) + LegalApiController (12 REST endpoints: ToS 3, SLA 2, AUP 2, Offboarding 3, Whistleblower 2 publicos anonimos) + JS behaviors + 4 unit tests. **jaraba_privacy:** JS behaviors (cookie-banner, dpa-signature, privacy-dashboard) + 8 SCSS partials + 4 unit tests. **ComplianceAggregatorService** (ecosistema_jaraba_core): Servicio transversal que agrega 9 KPIs de los 3 modulos en dashboard unificado (dpa_coverage, arco_pol_sla, cookie_consent_rate, tos_acceptance_rate, sla_compliance, aup_violations, backup_health, dr_test_coverage, status_page_uptime). Score global 0-100 con grade A-F. Alertas automaticas critico <50% / warning <80%. CompliancePanelController ruta `/admin/jaraba/compliance` + API `/api/v1/compliance/overview`. **jaraba_tenant_export fix:** Error 500 en /tenant/export resuelto — datos movidos a hook_preprocess_page() (zero-region templates skip page.content). 24 SCSS partials compliance (8 por modulo). 13 test files nuevos. Seccion 1.4 actualizada con Stack Compliance Legal N1. Arquitectura v36.0.0, Indice v53.0.0. 87 aprendizajes |
| 2026-02-16 | **36.0.0** | **FASE 11 Fiscal Integration Layer + Stack Compliance Legal N1 Implementation:** Dos entregas mayores. **1) FASE 11 completada** — Capa de integracion fiscal (9 entregables F11-1 a F11-9): FiscalComplianceService (score 0-100, 5 factores x 20 pts), FiscalDashboardController (/admin/jaraba/fiscal), page--fiscal.html.twig zero-region, KPI card en Admin Center + slide-panel, FiscalInvoiceDelegationService (deteccion B2G/B2B/nacional por NIF), 3 MJML email alerts fiscales (FISC_001-003), 9 features en FEATURE_ADDON_MAP, design token config fiscal (17 tokens), 38 tests (unit+kernel+functional). Stack fiscal F0-F11 COMPLETO (3 modulos + integration layer, 110 tests totales). **2) Stack Compliance Legal N1 implementado** — 3 modulos nuevos (198 archivos, +13,281 LOC): `jaraba_privacy` (GDPR/DPA, 5 entities, 5 services), `jaraba_legal` (ToS/SLA/AUP, 6 entities, 5 services), `jaraba_dr` (DR, 3 entities, 5 services). 3 zero-region page templates + 3 compliance partials + 36 SVG icons. LegalResolution entity: string→string_long para JSON fields. Auditoria N1: 12.5%→95%+. Arquitectura v35.0.0, Indice v52.0.0. 86 aprendizajes |
| 2026-02-16 | **35.0.0** | **Plan Implementacion Stack Compliance Legal N1:** Plan de implementacion completo para 3 modulos del nivel N1 Foundation (docs 183, 184, 185). **jaraba_privacy** (GDPR DPA + LOPD-GDD): 5 Content Entities (DpaAgreement, PrivacyPolicy, CookieConsent, ProcessingActivity, DataRightsRequest + SecurityBreach), 5 Services, 10 API endpoints. **jaraba_legal** (Legal Terms SaaS): 6 Content Entities (LegalDocument, SlaDefinition, SlaCredit, OffboardingRequest, WhistleblowerReport, LegalAcceptance), 5 Services (TosManager, SlaCalculator, OffboardingOrchestrator, WhistleblowerManager, LegalDocumentRenderer), 12 API endpoints. **jaraba_dr** (Disaster Recovery): 3 Content Entities (DrTestResult, BackupRecord, DrCommunicationLog), 5 Services (BackupVerifier, FailoverOrchestrator, DrTestRunner, DrCommunicationService, DrReportGenerator), 8 API endpoints. 13 fases (F0-F12), 91-118h / 4,095-5,310 EUR. Tabla correspondencia 40+ filas a docs 183/184/185/201. 20 directrices cumplidas. Auditoria N1 target: 12.5%→95%+. Plan: `20260216-Plan_Implementacion_Stack_Compliance_Legal_N1_v1.md`. Indice v51.0.0 |
