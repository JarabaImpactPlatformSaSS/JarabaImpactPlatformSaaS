...
**Versión:** 176.0.0 (WhatsApp Cloud API activado producción + WHATSAPP-WEBHOOK-SSOT-001 + 6 Supervisor workers, aprendizaje #237)
...
| **Supervisor** | 4.2.5 | 6 workers (5 AI + 1 WhatsApp), process management |
...
| **Viewport dvh fallback** | VIEWPORT-DVH-001 | TODA declaracion `height: 100vh` o `min-height: 100vh` DEBE ir seguida de la linea equivalente con `dvh` (`min-height: 100dvh`). Progressive enhancement — browsers sin soporte ignoran dvh. 93.5% soporte global. Validacion: `validate-viewport-dvh.php` (pre-commit + CI run_check) | P0 |
| **Nowrap overflow protection** | NOWRAP-OVERFLOW-001 | TODA declaracion `white-space: nowrap` DEBE tener `overflow: hidden; text-overflow: ellipsis;` en las 5 lineas siguientes, O responsive wrap `@media (max-width: 479px) { white-space: normal; }`, O comentario `// NOWRAP-SAFE: [razon]`. Validacion: `validate-nowrap-overflow.php` (CI warn) | P1 |
| **Fixed stacking mobile** | FIXED-STACKING-MOBILE-001 | Elementos con `position: fixed` en zona inferior DEBEN ser conscientes de bottom-nav (`.has-bottom-nav`). FABs suman `calc(56px + env(safe-area-inset-bottom, 0) + var(--ej-spacing-md))` a su `bottom`. Maximo 1 FAB simultaneo en movil | P1 |
| **Imagenes SuccessCase seed** | IMG-SEED-001 | Imagenes de SuccessCase entity se seedean desde theme fallback images via `seed-success-case-images.php` (idempotente). Integrado en deploy.yml post content-seed. Archivos `public://` NO se versionan en git — el seed los crea en runtime | P1 |
| **Image dimensions CLS** | IMG-DIMENSIONS-001 | TODA etiqueta `<img>` en templates Twig DEBE incluir atributos `width` y `height` (o usar parcial `_responsive-image.html.twig`). Previene CLS > 0.1 en Core Web Vitals. Validacion: `validate-img-dimensions.php` (CI warn) | P1 |
...
| **Circuit Breaker Email SES→IONOS** | EMAIL-FAILOVER-001 | EmailFailoverService implementa circuit breaker: CLOSED (normal, SES) → 3 failures consecutivos → OPEN (15min cooldown, usa IONOS) → HALF-OPEN (reintenta SES). hook_mailer_post_render() setea DSN dinámicamente via _jaraba_ses_transport_build_dsn(). hook_mailer_post_send() resetea el breaker en éxito. Estado visible en /admin/reports/status. | P0 |
| **ConfigEntity Override Limitation** | CONFIG-ENTITY-OVERRIDE-001 | `$config[]` overrides en settings.php SOLO aplican a `\Drupal::config()` reads. ConfigEntity::load() devuelve propiedades originales SIN override. Para inyectar secrets en transport entities o similares, construir valores desde getenv() directamente en código PHP, NO depender de $config overrides. | P0 |
| **Email Reputation Monitor** | EMAIL-REPUTATION-MONITOR-001 | EmailReputationMonitorService ejecuta cron cada 6h. Thresholds: bounce rate >5% = WARNING (AWS pausa al 10%), complaint rate >0.1% = CRITICAL (AWS pausa al 0.5%). Métricas en hook_requirements(). Auditoría semanal de supresión. | P1 |
...
| **URLs resueltas en PHP, no en Twig** | TWIG-URL-RESOLVE-PHP-001 | En sitio multilingüe con prefijo de idioma, las URLs de rutas internas NUNCA se construyen en Twig concatenando paths (ej. `{{ prefix }}/caso-de-exito/{{ slug }}`). SIEMPRE resolver en PHP con `Url::fromRoute()` y pasar como string al template (ej. `testimonial_urls`, `case_study_url`). Razón: `Url::fromRoute()` aplica automáticamente el prefijo `/es/`, `/en/`, etc. Los paths hardcodeados lo saltan y causan 404. En partials con `only`: la URL resuelta DEBE pasarse explícitamente en el include (TWIG-INCLUDE-ONLY-001). Patrón preprocess: generar mapa `slug → url` en preprocess y acceder como `{{ testimonial_urls['slug'] }}`. Validación: `php scripts/validation/validate-twig-hardcoded-routes.php` | P0 |
...
| Fecha | Versión | Descripción |
|-------|---------|-------------|
| 2026-03-28 | **175.0.0** | **WhatsApp Cloud API Activación Producción (Aprendizaje #236):** 1 regla nueva: WHATSAPP-WEBHOOK-SSOT-001 (P0, jaraba_whatsapp es el SSOT para /api/v1/whatsapp/webhook — ruta de AgroConecta eliminada para evitar colisión que causaba que AgroConecta capturara GET+POST antes que jaraba_whatsapp). Activación completa: Meta Cloud API conectada (App ID 1577739443312608, WABA 2433356897133166, Phone +34 623 27 43 04), Supervisor worker jaraba-whatsapp (sleep 30s, prioridad 150), Redis queue routing whatsapp_process_message, deploy.yml inyecta APP_SECRET desde GitHub Secrets, webhook verificado E2E en producción. Fix tests: WhatsAppConversationAgent type hint object (BaseAgent compat), WaMockAiProviderInterface (MOCK-METHOD-001). Fix permisos: settings.env.php necesita root:www-data 640 para que PHP-FPM lea env vars. OPcache: tras drush en con validate_timestamps=0, SIEMPRE php-fpm reload. 6 Supervisor workers totales (5 AI + 1 WhatsApp). Regla de oro #166: Las colisiones de rutas en Drupal se resuelven por orden alfabético de módulo — si dos módulos registran el mismo path, el primero alfabéticamente gana silenciosamente. Aprendizaje #236: PHP convierte dots a underscores en $_GET (hub.mode→hub_mode) pero OPcache con validate_timestamps=0 puede servir un controller obsoleto indefinidamente — tras habilitar un módulo en producción, systemctl reload php8.4-fpm es obligatorio. |
| 2026-03-28 | **173.0.0** | **SES Operativo + Circuit Breaker + Reputation Monitor (Aprendizaje #234):** 2 reglas nuevas: EMAIL-FAILOVER-001 (P0, circuit breaker SES→IONOS — EmailFailoverService con 3 estados CLOSED/OPEN/HALF-OPEN, 3 failures → 15min cooldown en IONOS, hook_mailer_post_render() routing dinámico via _jaraba_ses_transport_build_dsn(), hook_mailer_post_send() resetea breaker en éxito, hook_requirements() muestra estado en status report), CONFIG-ENTITY-OVERRIDE-001 (P0, $config[] overrides en settings.secrets.php SOLO aplican a \Drupal::config() reads — ConfigEntity::load() devuelve valores originales sin override, credenciales SMTP DEBEN construirse desde getenv() directamente al generar DSN). 1 regla actualizada: EMAIL-REPUTATION-MONITOR-001 (P1, EmailReputationMonitorService con cron 6h — thresholds bounce >5% WARNING, complaint >0.1% CRITICAL, metrics en hook_requirements(), audit semanal supresión). Default transport cambiado de smtp_ionos a smtp_ses en producción. IAM user rotado a v2 (jaraba-ses-smtp-prod-v2). Email premium template verificado E2E (email-wrap.html.twig branded con header gradient + logo + footer). 197 validators (193 pass, 3 fail pre-existentes). Regla de oro #165: Los $config[] overrides de Drupal en settings.php son una herramienta de simple config — para ConfigEntities, el único mecanismo fiable de inyectar secrets es construir valores directamente desde getenv() en código PHP. Aprendizaje #234: Symfony Mailer Drupal construye el DSN via MailerTransport::load()→getDsn()→plugin→$this->configuration — pero las propiedades de ConfigEntity NO reciben los $config overrides de settings.php (solo \Drupal::config() sí). Esto causaba 530 Auth Required porque user/pass estaban vacíos en el DSN. Fix: _jaraba_ses_transport_build_dsn() construye DSN completo desde getenv(). |
| 2026-03-27 | **172.0.0** | **AWS SES Email Transport + Bounce Suppression (Aprendizaje #233):** 3 reglas nuevas: EMAIL-DEDICATED-IP-001 (P0, transporte email transaccional dedicado via Amazon SES SMTP eu-central-1 Frankfurt — IP compartida SES separada de IONOS, DKIM Easy DKIM RSA 2048-bit verificado, MAIL FROM personalizado mail.plataformadeecosistemas.com con MX+SPF en IONOS DNS, credenciales SMTP IAM jaraba-ses-smtp-prod via getenv() SECRET-MGMT-001, transport symfony_mailer smtp_ses configurable sin code deploy), EMAIL-BOUNCE-SYNC-001 (P0, supresión automática de bounces/complaints via SNS→webhook — SesWebhookController en /api/v1/ses/webhook procesa SubscriptionConfirmation+Bounce+Complaint+Delivery, EmailSuppressionService con tabla email_suppression: hard bounces permanentes, complaints permanentes, transient bounces 24h cooldown, hook_mail_alter() check pre-send, getStats() para monitoring), EMAIL-SES-TRANSPORT-001 (P1, validador 10 checks: module structure, SMTP transport config, webhook route, suppression methods, mail_alter hook, schema table, controller SNS types, secrets config, core.extension, .env.example). Arquitectura email dual: IONOS SMTP para correo personal/empresarial (MX @ sin cambios), AWS SES para transaccional Drupal (notificaciones, resets, onboarding). DNS IONOS: 2 registros nuevos subdominio `mail` (MX→feedback-smtp.eu-central-1.amazonses.com, TXT SPF amazonses.com), 3 CNAME DKIM ya existentes. SNS topic jaraba-ses-notifications con suscripción HTTPS webhook. Solicitud producción SES caso 177461806800544 (sandbox→producción). 7 ficheros modificados/creados: config/sync/symfony_mailer.mailer_transport.smtp_ses.yml, config/sync/core.extension.yml (+jaraba_ses_transport), config/deploy/settings.secrets.php (SES_SMTP_*), .env + .env.example, validate-ses-transport-integrity.php (reescrito 10 checks SMTP). Regla de oro #164: Un sistema email de producción SaaS DEBE separar IP de envío transaccional (SES) de la IP de correo empresarial (IONOS) — un spike de bounces en notificaciones automáticas NO debe contaminar la reputación del dominio para correo personal. Aprendizaje #233: Las credenciales SMTP de SES NO son IAM Access Keys normales — SES Console → Create SMTP Credentials genera un par username (AKIA*) + password derivada por HMAC-SHA256 del Secret Key. El SMTP username coincide en formato con el Access Key ID pero la password es un valor derivado diferente de 44 caracteres base64. |
| 2026-03-27 | **169.0.0** | **Rebrand Desarrollo Local + 3 Salvaguardas Deploy (Aprendizaje #230):** 3 reglas nuevas: BRAND-TERM-VALIDATOR-001 (run_check, validator que detecta terminos de marca obsoletos "Desarrollo Rural" en frontend, 9174 ficheros escaneados con exclusion inteligente de nombres propios — titulos academicos UCO, GDR Campina Sur, LEADER EU — via patron match + post-filter context), CONTENT-SEED-DEPLOY-001 (P1, nuevo step en deploy.yml post-config:import que ejecuta automaticamente import-metasite-content.php + validate-content-sync.php + populate-metasite-translations.php — cierra el gap critico entre JSON commitado y BD produccion que causaba que cambios en canvas_data no se propagaran), TRANSLATION-SYNC-DEPLOY-001 (P1, drush locale:check + locale:update automatico en deploy — cierra el gap de traducciones .po que no se propagan via drush cim porque locales_target.customized=1 son DATOS no CONFIG). Rebrand completo "Desarrollo Rural" → "Desarrollo Local" en 19 ficheros, 3 idiomas (ES/EN/PT-BR), Schema.org JSON-LD, canvas_data, content-seed, scripts i18n/maintenance. ~50 reemplazos. 6 nombres propios preservados (titulos academicos UCO, organizacion GDR). Bio Remedios Estevez reestructurada: Sociedad Municipal → "prestacion de servicios publicos locales". Propagado a produccion: 75 entidades actualizadas, 45 traducciones, 11513 locale strings, 45/45 validaciones OK. 180 scripts validacion (127 run + 56 warn). Regla de oro #163: Un rebrand terminologico en un SaaS multi-tenant multi-idioma requiere verificar 7 capas: PHP → Twig → i18n .po → i18n .js → Config YML → Content-seed JSON → BD canvas_data. Si solo cambias el codigo sin propagar a BD, el usuario sigue viendo el texto antiguo. Aprendizaje #230: La propagacion de contenido L2 (canvas_data) de local a produccion NO es automatica — el deploy.yml solo hace config:import (L1) pero no content-seed import. Este gap invisible causa que cambios en JSONs commitados nunca lleguen a la BD de produccion hasta que alguien ejecute manualmente import-metasite-content.php. CONTENT-SEED-DEPLOY-001 cierra este gap. |
