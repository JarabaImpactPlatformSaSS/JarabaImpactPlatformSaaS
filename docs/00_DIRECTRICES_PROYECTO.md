...
**Versión:** 155.0.0 (CASE-STUDY-PATTERN-001 + 9 case studies verticales + validate-case-study-completeness + 73 scripts + aprendizaje #207 + regla de oro #145)
...
| **Case Study Landing Pattern** | CASE-STUDY-PATTERN-001 | Cada vertical comercial DEBE tener una landing de caso de éxito con: (1) Controller en `{module}/src/Controller/{Vertical}CaseStudyController.php` con `#pricing` variable (NO-HARDCODE-PRICE-001), (2) Template `{vertical}-case-study.html.twig` con 10 secciones (hero, contexto, dolor, descubrimiento, timeline, resultados, clímax, testimonial, pricing, CTA final), (3) hook_theme entry `{vertical}_case_study`, (4) Library `{vertical}-case-study` reusando `jarabalex-case-study.css`, (5) 6 imágenes WebP, (6) Ruta `.case_study.{protagonist}`, (7) Schema.org JSON-LD, (8) CTAs con data-track-cta + data-track-position. Módulos con `str_starts_with` en preprocess_page/theme_suggestions_page_alter DEBEN excluir `.case_study.` routes. B2G (Andalucía EI) usa CTA "Solicitar demo" sin mini-pricing. 9/9 comerciales cubiertos (Demo excluido = interno). Validación: `php scripts/validation/validate-case-study-completeness.php` | P1 |
