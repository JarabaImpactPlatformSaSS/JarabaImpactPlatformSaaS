...
**Version:** 68.0.0 (Stripe Embedded Checkout: STRIPE-CHECKOUT-001, checkout self-service ui_mode embedded, sync Product/Price bidireccional + aprendizaje #165)
...
70. **Core Web Vitals tracking con PerformanceObserver nativo:** El theme carga `cwv-tracking.js` globalmente. Usa PerformanceObserver para LCP, CLS (session window), INP (event duration), FCP, TTFB. Reporta via `window.dataLayer.push({event: 'cwv_metric', cwv_name, cwv_value, cwv_rating})`. Thresholds: LCP [2500,4000], CLS [0.1,0.25], INP [200,500], FCP [1800,3000], TTFB [800,1800]. NO depende de librerias externas (web-vitals.js). Hero images DEBEN tener `fetchpriority="high" decoding="async"` y slider first-slide `loading="eager"`.
71. **Imagenes responsive con cascada AVIF > WebP > fallback:** Usar `<picture>` con `<source type="image/avif">` primero, `<source type="image/webp">` segundo, `<img>` fallback. La funcion Twig `responsive_image(url, options)` genera el markup completo con srcset breakpoints (400w, 800w, 1200w) via ImageStyle. Usar `_responsive-image.html.twig` partial en templates que necesiten picture element. AVIF tiene ~50% menor peso que WebP para fotos.
72. **Concurrent edit locking con optimistic locking:** Entidades editables en canvas (PageContent) DEBEN tener campos `edit_lock_uid` + `edit_lock_expires` (NOT revisionable). Al guardar, verificar header `X-Entity-Changed` contra `$entity->getChangedTime()` — si mismatch, retornar 409 Conflict. JS: acquire lock al init, heartbeat cada 2 min, release en beforeunload con `keepalive: true`. Lock saves usan `setSyncing(TRUE)` para evitar revisiones innecesarias.
73. **CSS code splitting via Drupal libraries route-specific:** Extraer SCSS route-specific de main.scss a bundles compilados independientes. Registrar como libraries en `{theme}.libraries.yml`. Adjuntar via `hook_page_attachments_alter()` con mapa de prefijo de ruta → array de libraries. Permite cargar solo el CSS necesario por pagina. Bundles actuales: ai-dashboard, page-builder-dashboard, content-hub, auth, employability-pages, jobseeker-dashboard, agent-dashboard.
74. **MultiModal AI completo con guardrails pre-generacion:** `MultiModalBridgeService` implementa 4 operaciones: `analyzeImage()` (vision), `transcribeAudio()` (STT), `synthesizeSpeech()` (TTS), `generateImage()` (DALL-E). Las operaciones de OUTPUT (TTS, ImageGen) DEBEN pasar guardrails + jailbreak check en el prompt ANTES de enviar al proveedor. Ficheros generados en `temporary://jaraba-{type}/`. Usar `resolveProviderForOperation()` para obtener el proveedor correcto por OperationType.
75. **Agent benchmarks con LLM-as-Judge automatizado:** `AgentBenchmarkService::runBenchmark()` ejecuta test cases contra un agente y evalua con `QualityEvaluatorService` (LLM-as-Judge). Resultados almacenados en `AgentBenchmarkResult` entity. `compareVersions()` ejecuta ambas versiones con mismos inputs y retorna delta de scores. Test cases: `['input' => ..., 'expected_output' => ..., 'criteria' => [...]]`.
76. **PersonalizationEngine como orquestador de N fuentes:** `PersonalizationEngineService::getRecommendations()` consume N servicios existentes de recomendacion (contenido, empleo, formacion, herramientas, etc.) con scoring ponderado por contexto. Cada fuente es opcional: si falla, usa las demas (fallback graceful). Re-ranking por engagement historico del usuario. NO implementar pipeline ML real — usar heuristicas inteligentes + datos de uso existentes.
77. **SCSS compilado SIEMPRE verificado tras edicion (SCSS-COMPILE-VERIFY-001):** Tras editar CUALQUIER archivo `.scss`, SIEMPRE recompilar el CSS antes de considerar el trabajo completo. Verificar con `ls -la css/ scss/` que el timestamp del `.css` compilado es posterior al SCSS mas reciente. El gap mas comun entre "el codigo existe" y "el usuario lo experimenta" es SCSS editado pero CSS no recompilado — el usuario ve la pagina con estilos viejos o rotos. Aplica a TODOS los modulos con carpeta `scss/`. Comando: `npx sass scss/main.scss:css/output.css --style=compressed`.
78. **Verificacion runtime end-to-end obligatoria post-implementacion (RUNTIME-VERIFY-001):** Al completar una feature, verificar las 5 dependencias RUNTIME: (1) CSS compilado sincronizado con SCSS, (2) tablas DB definidas en `hook_schema()` / update hooks, (3) rutas referenciadas existen en `.routing.yml`, (4) selectores `data-*` en templates coinciden con los que busca el JS (`document.querySelector`), (5) `drupalSettings` inyectados por el controller (render array `#attached.drupalSettings`) coinciden exactamente con lo que JS lee (`drupalSettings.{key}`). Esta verificacion evita el patron donde el codigo pasa revision pero el usuario experimenta funcionalidad rota.
79. **Queries directas SIEMPRE contra _field_data para entidades traducibles (TRANSLATABLE-FIELDDATA-001):** Cuando una ContentEntity tiene `translatable = TRUE`, los campos viven en `{entity_type}_field_data`, NO en la tabla base. La tabla base solo tiene `id`, `uuid`, `langcode`. Queries directas con `$database->select('support_ticket', 't')` fallan con "Column not found" para CUALQUIER campo custom. Usar `select('support_ticket_field_data', 't')`. Entity queries (`getStorage()->getQuery()`) manejan esto automaticamente — preferirlas cuando no se necesitan funciones SQL agregadas (`AVG`, `SUM`, `DATE_FORMAT`). Tests con mocks que retornan `willReturnSelf()` para `addExpression()` y `join()` crean falsa confianza — estos metodos retornan strings en Drupal real, NO `$this`. Agregar tests de contrato que verifiquen el nombre de tabla.
80. **NUNCA encadenar addExpression() ni join() en queries Drupal (QUERY-CHAIN-001):** `Select::addExpression()` y `Select::join()` retornan un string alias (el nombre del campo/tabla), NO `$this`. La cadena `->addExpression('AVG(...)', 'avg')->execute()` llama `execute()` sobre el string `'avg'` → fatal error. La cadena `->join('table', 't', ...)->condition(...)` llama `condition()` sobre el string `'t'` → fatal error. SIEMPRE usar sentencias separadas: `$query->addExpression(...)` + `$query->execute()`. Los mocks de PHPUnit con `willReturnSelf()` ocultan este bug completamente — el test pasa pero el runtime explota.
81. **Aritmetica datetime vs timestamp en queries directas (DATETIME-ARITHMETIC-001):** Campos Drupal `datetime` (`datetime_type = 'datetime'`) almacenan VARCHAR `'Y-m-d\TH:i:s'` (ej: `'2026-02-27T10:30:00'`). Campos `created`/`changed` almacenan INT Unix timestamp. En MySQL, `varchar_field - int_field` convierte el varchar a su porcion numerica inicial (ej: `'2026-02-27T...'` → `2026`). La aritmetica `AVG(t.resolved_at - t.created)` produce basura. Fix: `UNIX_TIMESTAMP(REPLACE(t.resolved_at, 'T', ' '))` convierte datetime a Unix. Para comparaciones, usar formato ISO: `(new \DateTime(...))->format('Y-m-d\TH:i:s')`. Para null checks, usar `$query->isNotNull('t.field')` en vez de `condition('t.field', 0, '>')`.
82. **Opt-in explicito con Twig default(false) para features condicionales:** Variables Twig que controlan visibilidad de features DEBEN usar `|default(false)` para ser opt-in explicitas. La inyeccion se hace desde `preprocess_page()` con logica condicional (ej: `empty($variables['meta_site'])` → SaaS principal activa megamenu, meta-sitios conservan nav_items). NUNCA usar `default(true)` para features que dependen de configuracion externa — un template cargado sin el preprocess activo activaria la feature por error.
83. **Machine names → labels con campo _raw separado:** Datos internos (machine names como `ai_copilot`, `advanced_analytics`) NUNCA se exponen al usuario final. Usar metodo privado `formatFeatureLabels()` con mapa de 28+ machine names → `$this->t('Label Legible')`. Aplicar tanto a `features` en getPricingPreview() como a `features_highlights` en getFromPrice(). Conservar campo `features_raw` con machine names originales para logica interna (`hasFeature()` checks). Verificar coherencia labels↔machine names al anadir features nuevas.
84. **Auditar TODOS los valores de color/spacing en SCSS nuevos contra var(--ej-*, fallback) ANTES de marcar como completado (CSS-VAR-ALL-COLORS-001):** Todo valor visual en SCSS satelite DEBE estar envuelto en `var(--ej-*, fallback)`. NUNCA hex hardcodeado (`#ffffff`), `rgba()` sin wrapper, ni valores numericos de border-radius/shadow sin token. Patron correcto: `color: var(--ej-text-on-primary, #ffffff)`, `background: var(--ej-color-primary-light, rgba(255, 140, 66, 0.08))`, `border-radius: var(--ej-border-radius-pill, 20px)`. Los tokens custom usan fallback como valor por defecto hasta que se inyecten desde Theme Settings UI. Verificar con `grep -n '#[0-9a-fA-F]\{3,6\}' scss/components/_nuevo.scss` antes de compilar.
85. **jaraba_icon() en Twig SIEMPRE con color de paleta Jaraba (ICON-COLOR-001 + ICON-DUOTONE-001):** El parametro `color` DEBE especificar un nombre de paleta Jaraba (`naranja-impulso`, `azul-corporativo`, `verde-innovacion`, `white`, `neutral`). El default "current color" NO es fiable cuando el SVG se renderiza como `<img>` tag. Templates premium DEBEN usar `variant: 'duotone'` por defecto. Signature real: `jaraba_icon(category, name, { variant: 'duotone', color: 'naranja-impulso', size: '24px' })`. Funcion PHP: `JarabaTwigExtension::renderIcon(string $category, string $name, array $options = [])`.
86. **entity_reference target_id=0 almacena NULL en BD — usar notExists() para idempotencia:** En Drupal, un entity_reference con target_id=0 almacena NULL en la base de datos. Para queries de idempotencia (ej: seed data), NUNCA usar `->condition('field', 0)` — usar `->notExists('field')`. Descubierto en update hooks de seed data donde FAQs con tenant_id omitido (target_id=0) se duplicaban porque la condition no matcheaba NULL.
87. **Busqueda unificada cross-entity con hasDefinition() + Url::fromRoute():** APIs de busqueda que consultan multiples entity types DEBEN usar `$entityTypeManager->hasDefinition('type')` como guard para entity types opcionales (cross-modulo). TODAS las URLs de resultados DEBEN generarse con `Url::fromRoute()` — NUNCA construir URLs en JavaScript como fallback. Distinguir tipos de resultado con campo `type` para autocompletado. KB articles usan slug-based URLs.
88. **ControllerBase::$entityTypeManager — NUNCA readonly en constructor promotion (CONTROLLER-READONLY-001):** En Drupal 11, `ControllerBase::$entityTypeManager` tiene declaracion `protected` sin tipo ni readonly. Los subclasses que usan constructor promotion con `protected readonly EntityTypeManagerInterface $entityTypeManager` causan `Cannot redeclare non-readonly property as readonly`. Solucion: quitar del constructor promotion y asignar manualmente en el body: `$this->entityTypeManager = $entityTypeManager;`. Aplica a TODOS los controllers que inyecten entityTypeManager.
89. **Unificacion de landings verticales — 301 redirect + actualizar TODAS las referencias (LEGAL-LANDING-UNIFIED-001):** Cuando un vertical tiene multiples landings con features solapadas, unificar en la landing principal con 301 via `Url::fromRoute()` (NUNCA hardcodear paths por ROUTE-LANGPREFIX-001). Actualizar TODAS las referencias en un cambio coordinado: megamenu, redirects relacionados, SEO meta, Schema.org, $vertical_routes y $landing_routes en theme.
90. **LCIS 9 capas fail-open — KB como SSOT normativa, authority-aware RAG formula:** El Legal Coherence Intelligence System (9 capas L1-L8 + MT) usa fail-open en cada capa — si una capa falla se asume comportamiento por defecto seguro. `LegalCoherenceKnowledgeBase` centraliza TODA la logica normativa como constantes estaticas (NORMATIVE_HIERARCHY, NORM_TYPE_PATTERNS, FORAL_LAW_REGIMES). NUNCA duplicar regex o niveles en otros servicios. RAG ranking: `final_score = 0.55*semantic + 0.30*authority + 0.15*recency`.
91. **Hooks Claude Code — file_path directo de tool_input, deny JSON en stdout:** PreToolUse hooks leen JSON stdin via python3, extraen `tool_input.file_path` directo — NUNCA navegar dentro de `content` (es string en Write tool, causa AttributeError). Archivos bloqueados con JSON `{"permissionDecision": "deny"}` en **stdout** + exit 2 — NUNCA enviar a stderr porque Claude Code parsea stdout para el motivo. PostToolUse lint: errores a stdout con exit 2 para feedback directo. Lint < 5s por archivo.
92. **Elevacion landing sistematica 3 niveles — contenido + Schema.org + config layer:** Nivel 1: contenido landing reflejando infraestructura REAL del codebase (features/FAQs/stats con datos verificables, URLs via Url::fromRoute(); homepage es template-driven con Twig partials y fallback data, deteccion via isFrontPage()). Nivel 2: Schema.org SoftwareApplication JSON-LD con featureList completo + meta + OG + Twitter en PageAttachmentsHooks. Nivel 3: config layer FreemiumVerticalLimit + SaasPlan + PlanFeatures coherentes.
93. **METRICS-HONESTY — stats y metricas SOLO datos verificables del codebase:** NUNCA fabricar numeros de usuarios, clientes o conversiones. Solo datos verificables desde el codebase o documentacion: 10 verticales (VERTICAL-CANONICAL-001), 11 agentes IA Gen 2, 80+ modulos custom. Directiva existente en `_stats.html.twig` del homepage. Aplica a landings, pricing, demos y cualquier superficie publica.
94. **Validacion arquitectonica cruzada YAML↔PHP obligatoria (ARCH-VALIDATE-001):** Los errores mas peligrosos en Drupal cruzan fronteras inter-archivo: services.yml inyecta `@logger.factory` pero el constructor espera `LoggerInterface` → TypeError en runtime; routing.yml referencia `Controller::method` que no existe → `drush cr` crash fatal; `->addExpression()->execute()` encadenado → crash en cada page load. PHPStan/PHPCS NO los detectan. Los 6 scripts de `scripts/validation/` usan auto-descubrimiento via glob (zero listas hardcoded): nuevos modulos, servicios, rutas y entidades se validan automaticamente. Integrados en pre-commit (--fast <3s), CI y deploy (--full). Para anadir un nuevo check: crear script en `scripts/validation/`, registrarlo en `validate-all.sh` (fast o full), verificar exit 0 contra baseline, commit.
95. **ECA plugins DEBEN tener event_name en la anotacion (ECA-EVENT-001):** El `event_name` en `@EcaEvent` annotation NO es opcional — sin el, ECA genera warnings en runtime al construir el event map, aunque `definitions()` lo declare. Las 3 fuentes de verdad (annotation, definitions(), event class) DEBEN ser coherentes. Detectado automaticamente por `validate-entity-integrity.php`.
96. **Corregir baseline antes de integrar validador en CI (BASELINE-CLEAN-001):** Al introducir un validador nuevo, SIEMPRE: (1) crear script, (2) ejecutar contra codebase existente, (3) corregir TODAS las violaciones pre-existentes, (4) verificar exit 0, (5) integrar en CI. Un validador que falla en la baseline invalida CI para todo el equipo y destruye la confianza en la herramienta. En esta iteracion: 6 violaciones pre-existentes corregidas (3 SettingsForm + 3 admin_permission) ANTES de integrar en CI/deploy.
97. **LCIS entities de soporte DEBEN vivir en los modulos correctos (LCIS-AUDIT-001 + LCIS-GRAPH-001):** Las entities de datos normativos (`legal_norm_relation`) viven en `jaraba_legal_knowledge` (donde vive LegalNorm, entity duena de las relaciones — ENTITY-FK-001 entity_reference mismo modulo). Las entities de audit trail IA (`legal_coherence_log`) viven en `jaraba_legal_intelligence` (donde vive el pipeline LCIS que las produce). NO crear modulo standalone `jaraba_lcis` — el LCIS es un subsistema distribuido entre ambos modulos, con 9 clases namespace-aisladas en `src/LegalCoherence/`. Ventaja: evita circular dependencies, reutiliza entities existentes, mantiene cohesion vertical. QueueWorkers (`NormativeUpdateWorker`, `NormVectorizationWorker`) en jaraba_legal_intelligence (cercanos al pipeline que los consume). Constructor args opcionales con `= NULL` para backward compatibility.
98. **ConfigEntities tambien necesitan hook_update_N() con installEntityType() (UPDATE-HOOK-REQUIRED-001 reforzado):** Drupal trackea definiciones de entity types tanto para ContentEntities como ConfigEntities. Sin hook_update_N(), drush updatedb no instala el entity type y el status report muestra "needs to be installed". Salvaguarda automatizada: CHECK 7 en validate-entity-integrity.php escanea 446 entities y detecta cualquier entity type (Content O Config) cuyo ID no aparezca en el .install del modulo. Patron obligatorio: hook_install() para modulos nuevos + hook_update_N() individual por entity type como respaldo. Descubierto con VerticalBrandConfig y ActivationCriteriaConfig.
99. **Verificar type hint del constructor ANTES de inyectar logger (LOGGER-DI-001):** `LoggerChannelFactoryInterface` necesita `@logger.factory` (llama `->get('channel')`). `LoggerInterface` necesita `@logger.channel.*` (recibe el canal directamente). Un mismatch causa `LoggerChannel::get() undefined method` en runtime — el sitio arranca pero crashes al invocar el servicio. Descubierto en 12 servicios de 5 modulos (pixels, comercio_conecta, agroconecta, business_tools, resources). NUNCA hacer replace_all masivo sin verificar cada constructor individualmente.
100. **Argumentos en services.yml DEBEN coincidir exactamente con parametros del constructor (CIRCULAR-DEP-001 + PHANTOM-ARG-001):** Argumentos `@?service` en services.yml que el constructor PHP no acepta crean dependencias circulares silenciosas que solo se manifiestan en `drush cr` con error "Circular reference detected". Root cause: en algun momento se anadieron servicios al YAML sin verificar que el constructor los acepta. Descubierto en 6 servicios de 6 modulos. Salvaguarda recomendada: ampliar `validate-services-di.php` con conteo de parametros constructor vs argumentos YAML.
101. **checkAccess() declara AccessResultInterface, NO AccessResult (ACCESS-RETURN-TYPE-001):** En AccessControlHandler, el metodo `checkAccess()` DEBE declarar `AccessResultInterface` como return type, NO `AccessResult`. La clase padre `EntityAccessControlHandler::checkAccess()` declara `AccessResultInterface`. PHPStan Level 6 detecta el mismatch como "Return type AccessResult is not covariant". Afecta a 130+ AccessControlHandlers. Patron correcto: `protected function checkAccess(EntityInterface $entity, $operation, AccountInterface $account): AccessResultInterface`.
102. **Tagged registry pattern: CompilerPass + tagged services = extensibilidad sin tocar core (TENANT-SETTINGS-HUB-001):** `TenantSettingsRegistry` + `TenantSettingsSectionPass` CompilerPass recolecta servicios tagged `ecosistema_jaraba_core.tenant_settings_section`. Nuevas secciones se registran solo con tag en services.yml sin tocar core. 6 secciones iniciales: Domain/Plan/Branding/Design/ApiKeys/Webhooks.
103. **Verticales como addons componibles (ADDON-VERTICAL-001):** `addon_type='vertical'` + `vertical_ref` campo string en Addon entity. `TenantVerticalService` resuelve primario del grupo + addons activos por tenant. `FeatureAccessService` consulta `features_included` JSON con fallback legacy. `BaseAgent::getAddonVerticalsContext()` enriquece prompts IA si TenantVerticalService disponible via `hasService()`.
104. **Cada hostname multi-tenant necesita Domain entity (DOMAIN-ROUTE-CACHE-001):** RouteProvider cachea route collections en Redis por `[domain]` key. Sin Domain entity, el Domain module usa la default, colisionando cache keys. Cache HIT bypasa path processors completamente.
105. **Vary:Host prioridad -10 para CDN multi-tenant (VARY-HOST-001):** `SecurityHeadersSubscriber::onAddVaryHost()` prioridad -10 (DESPUES de FinishResponseSubscriber que establece Vary:Cookie en prioridad 0). Sin Vary:Host, CDN/Varnish sirve respuesta cacheada de un subdominio a otro. Mapas JS↔PHP verificar con grep contra form elements.
106. **FieldItemList en Twig = metodos getter obligatorios (TWIG-ENTITY-METHOD-001):** NUNCA acceder a campos de ContentEntity via property directa en Twig (`entity.name`). SIEMPRE usar metodos getter (`entity.getName()`). Property access devuelve FieldItemList no imprimible → error 500.
107. **Stripe Embedded Checkout: ui_mode embedded, NUNCA hosted redirect (STRIPE-CHECKOUT-001):** Checkout self-service usa `ui_mode: 'embedded'` con `return_url` (NUNCA `success_url`/`cancel_url` que son para hosted mode). `StripeProductSyncService` sincroniza `SaasPlan`→Stripe Product/Price idempotentemente con lock + archivado de precios obsoletos (Stripe Prices son inmutables). JS carga Stripe.js dinamicamente, obtiene CSRF token, POST para crear session, monta `initEmbeddedCheckout()`. Templates zero-region via `page--checkout.html.twig`. Pricing CTAs apuntan a checkout cuando `saas_plan_id` existe.
...
|-------|---------|-------------|
| 2026-03-05 | **68.0.0** | **Stripe Embedded Checkout Self-Service (STRIPE-CHECKOUT-001) Workflow:** 1 patron nuevo: STRIPE-CHECKOUT-001 — checkout self-service con Stripe Embedded Checkout (`ui_mode: 'embedded'`). Flujo completo: (1) `StripeProductSyncService` sincroniza SaasPlan→Stripe Product/Price idempotentemente con LockBackend + archivado de precios obsoletos (Stripe Prices inmutables), (2) `CheckoutSessionService` crea session con `return_url` (NUNCA `success_url`/`cancel_url` que son hosted), (3) `CheckoutController` sirve templates zero-region via `page--checkout.html.twig`, (4) `stripe-checkout.js` carga Stripe.js dinamicamente, obtiene CSRF token, POST a createSession, monta `initEmbeddedCheckout()`, (5) `BillingWebhookController` procesa `checkout.session.completed` para auto-provisioning. Integration points: `MetaSitePricingService` expone `saas_plan_id` en `getPricingPreview()`, `pricing-page.html.twig` CTAs apuntan a checkout si plan sincronizado, `pricing-toggle.js` sincroniza `?cycle=` param, `ecosistema_jaraba_theme.theme` inyecta body class `page-checkout` + theme suggestion `page__checkout`. Regla de oro #107 (Stripe Embedded Checkout: ui_mode embedded + return_url, Prices inmutables = archivar viejo + crear nuevo, JS carga Stripe.js dinámicamente). Aprendizaje #165. |
| 2026-03-05 | **66.0.0** | **Remediacion Onboarding/Checkout 32 Gaps + Google OAuth + WCAG 2.1 AA Workflow:** 2 patrones nuevos: (1) TWIG-ENTITY-METHOD-001 — NUNCA acceder a campos de ContentEntity via property directa en Twig (entity.name, entity.id). SIEMPRE usar metodos getter (entity.getName(), entity.id()). Property access devuelve FieldItemList no imprimible → error 500. Descubierto en ecosistema-jaraba-register-form.html.twig al verificar Google OAuth. (2) GOOGLE-OAUTH-LIGHTWEIGHT-001 — OAuth Google implementado sin modulo contrib: GoogleOAuthService lee credenciales de settings.secrets.php, genera auth URL con CSRF state en session, intercambia code por user info, 3 escenarios (login/create/error). Verificacion: isConfigured() en runtime, curl 302 a accounts.google.com, boton visible en /registro/{vertical}. Workflow WCAG: skip navigation, focus-visible 3px outline, aria-describedby en inputs, role=alert en errores, touch targets 44px, semantic nav+ol para progress, prefers-reduced-motion. Workflow page--auth: preprocess switch/default ampliado para rutas de onboarding — extrae system_main_block cuando ruta es ecosistema_jaraba_core.onboarding.*. Chart.js canonical via ecosistema_jaraba_core/chartjs dependency. Regla de oro #106 (FieldItemList en Twig = metodos getter obligatorios). Aprendizaje #163. |
| 2026-03-05 | **65.0.0** | **Multi-Tenant Route Cache Isolation + Vary:Host + Visual Customizer Workflow:** 3 patrones nuevos: (1) DOMAIN-ROUTE-CACHE-001 — RouteProvider (core/lib/Drupal/Core/Routing/RouteProvider.php:168-194) cachea route collections en Redis bin `data` con cache key `route:[domain]=X:[language]=Y:/path`. La parte `[domain]` viene de DomainSubscriber::onRouteMatch() linea 106. Sin Domain entity para un hostname, el Domain module usa el default, colisionando cache keys. Cache HIT bypasa path processors completamente (linea 184 solo corre en ELSE). Cada hostname multi-tenant DEBE tener su propia Domain entity. (2) VARY-HOST-001 — SecurityHeadersSubscriber dual event: onKernelResponse prioridad 100 (security headers) + onAddVaryHost prioridad -10 (Vary:Host DESPUES de FinishResponseSubscriber que establece Vary:Cookie en prioridad 0). Sin Vary:Host, CDN/Varnish sirve respuesta cacheada de un subdominio a otro. (3) PHANTOM-ARG-JS-001 — Mapas JS de field names (colorMap, typographyMap, sizeMap en visual-customizer.js) DEBEN coincidir EXACTAMENTE con PHP form element keys de TenantThemeCustomizerForm. Drupal vertical-tabs.js usa `$summary[0]?.firstChild?.textContent` — HTML markup en #title produce string vacio. Usar $this->t() plain text. Reglas de oro #104 (cada hostname necesita Domain entity, sin ella route cache colision + path processors bypaseados), #105 (Vary:Host prioridad -10, mapas JS↔PHP verificar con grep contra form elements). Aprendizaje #162. |
| 2026-03-04 | **63.0.0** | **CI/PHPStan Fixes — ACCESS-RETURN-TYPE-001 Workflow:** 1 patron nuevo: ACCESS-RETURN-TYPE-001 — checkAccess() en AccessControlHandler DEBE declarar `AccessResultInterface` como return type (NO `AccessResult`). PHPStan Level 6 detecta return type covariance mismatch contra `EntityAccessControlHandler::checkAccess()`. Corregido en 130+ AccessControlHandlers. CI fitness-functions pipeline: 35→0 errores. PHPStan baseline: 1890 errores eliminados (~41K lineas). Kernel tests: 460 tests, 2243 assertions. Regla de oro #101 (checkAccess() return type AccessResultInterface). Aprendizaje #160. |
| 2026-03-04 | **62.0.0** | **Stripe Connect Integration + DI Audit Runtime Fixes Workflow:** 3 patrones nuevos: (1) LOGGER-DI-001 — verificar type hint constructor PHP antes de decidir entre `@logger.factory` (LoggerChannelFactoryInterface, llama ->get()) y `@logger.channel.*` (LoggerInterface, recibe canal directamente) — mismatch causa crash en runtime no en boot, 12 servicios corregidos en 5 modulos, (2) CIRCULAR-DEP-001 + PHANTOM-ARG-001 — argumentos @? en services.yml que el constructor no acepta crean ciclos de dependencia circular silenciosos, diagnostico: contar params PHP vs args YAML, 6 ciclos resueltos en 6 modulos (interactive indent YAML, core style_preset 4→2, foc metrics_calculator, email campaign_service, lms course+cert+xapi, resources subscription_service), (3) STRIPE-ENV-UNIFY-001 — settings.secrets.php como punto unico de inyeccion para multiples namespaces config (3: core.stripe, foc.settings, legal_billing.settings), cada modulo Stripe lee via $config override no getenv() directo (excepto fallback en StripeConnectService). Reglas de oro #99 (LOGGER-DI-001 verificar constructor type hint), #100 (args YAML = params constructor PHP). Aprendizaje #159. |
| 2026-03-05 | **64.0.0** | **Verticales Componibles + Tenant Settings Hub + Stripe Sync Workflow:** 3 patrones nuevos: (1) Verticales como add-ons componibles (Addon entity con addon_type='vertical' + vertical_ref campo string referenciando machine_name del vertical; TenantVerticalService resuelve primario del grupo + verticales addon activos por tenant; FeatureAccessService consulta addon_subscription entities para features_included JSON con fallback a TenantAddon legacy; BaseAgent::getAddonVerticalsContext() enriquece prompts IA con verticales addon si TenantVerticalService disponible via hasService()), (2) Tagged service registry extensible (TenantSettingsRegistry + TenantSettingsSectionPass CompilerPass que recolecta servicios tagged `ecosistema_jaraba_core.tenant_settings_section`; 6 secciones iniciales Domain/Plan/Branding/Design/ApiKeys/Webhooks; cada seccion implementa TenantSettingsSectionInterface con getId()/getLabel()/getWeight()/buildSection(); nuevas secciones se registran solo con tag en services.yml sin tocar core), (3) Stripe sync bidireccional con rollback (TenantSubscriptionService::changePlan() actualiza local + llama StripeSubscriptionService::updateSubscription(); si Stripe falla hace revert local manteniendo consistencia; lazy-load via getStripeSubscription() para romper circular dep TenantSubscription↔StripeSubscription; FeatureAccessService unificado con deps @? opcionales). Reglas de oro #102 (tagged registry pattern: CompilerPass + tagged services = extensibilidad sin tocar core), #103 (verticales como addons: addon_type='vertical' + TenantVerticalService multi-vertical resolution + AI context enrichment). Aprendizaje #161. |
| 2026-03-04 | **61.0.0** | **7 Items SaaS Post-Evaluacion — VerticalBrand + Pre-PMF + PilotManager + PIIL + OpenAPI + ConfigEntity Safeguards Workflow:** 2 patrones nuevos: (1) Revelacion progresiva multi-vertical (VerticalBrandConfig ConfigEntity gestiona sub-marca por vertical con 4 niveles revelacion landing/trial/expansion/enterprise; RevelationLevelService determina nivel segun plan del tenant; NavigationFilterService filtra menu items; VerticalBrandSeoService genera meta tags; FeatureGateRouterService centraliza despacho a 10 FeatureGateService verticales via ServiceLocator; Twig test 'feature_allowed' evalua feature gates directamente en templates), (2) ConfigEntity update hooks obligatorios (UPDATE-HOOK-REQUIRED-001 reforzado: ConfigEntities NO estan exentas de hook_update_N() con installEntityType() — Drupal trackea definiciones de entity types independientemente del storage backend; CHECK 7 en validate-entity-integrity.php escanea 446 entities y detecta entity types sin referencia en .install; salvaguarda automatizada integrada en scripts/validation/; patron: hook_install() para modulos nuevos + hook_update_N() individual por entity type). Modulo NUEVO jaraba_pilot_manager: 3 ContentEntities + PilotEvaluatorService + 5 ECA Events + PilotDashboardController. Extension jaraba_institutional: 13 campos PIIL/FUNDAE/FSE+ + PiilMetricsService + StoSyncService. Extension jaraba_integrations: OpenApiSpecService + OpenApiController. PHPStan L6: 95+ errores corregidos (patrones: empty()→strict comparison, assert()→PHPDoc var annotation, short ternary→null coalesce, EntityInterface→ContentEntityInterface). 40 tests, 294 assertions. Regla de oro #98 (ConfigEntities necesitan hook_update_N). Aprendizaje #158. |
| 2026-03-04 | **60.0.0** | **LCIS Gap Closure — EU AI Act Audit Trail + Normative Graph Entities + 2 QueueWorkers Workflow:** 1 patron nuevo: entities de soporte LCIS distribuidas entre modulos correctos (`legal_norm_relation` en jaraba_legal_knowledge donde vive LegalNorm con entity_reference mismo modulo, `legal_coherence_log` en jaraba_legal_intelligence donde vive el pipeline LCIS; NO crear jaraba_lcis standalone — subsistema distribuido con 9 clases namespace-aisladas en src/LegalCoherence/; QueueWorkers NormativeUpdateWorker+NormVectorizationWorker en jaraba_legal_intelligence cercanos al pipeline; constructor args opcionales `= NULL` para backward compatibility; NormativeGraphEnricher L3 consulta grafo real de relaciones en vez de heuristicas basadas en payload.status; Validator L6 + Verifier L7 crean audit log fail-open via EntityTypeManager opcional). Regla de oro #97 (LCIS entities en modulos correctos: datos normativos en legal_knowledge, audit IA en legal_intelligence, NO modulo standalone). Aprendizaje #157. |
| 2026-03-03 | **59.1.0** | **Correccion integridad golden rules:** Auditoria de coherencia documental detecto 3 anomalias en la lista numerada de Seccion 5: (1) Reglas #86-93 existian SOLO en changelogs (v54-v58) pero NUNCA fueron insertadas en la lista numerada — insertadas 8 reglas: #86 entity_reference NULL/notExists, #87 busqueda unificada cross-entity, #88 CONTROLLER-READONLY-001, #89 LEGAL-LANDING-UNIFIED-001, #90 LCIS 9 capas, #91 hooks Claude Code, #92 landing elevation 3 niveles, #93 METRICS-HONESTY. (2) Regla #47 (copilot URL action buttons) estaba posicionada despues de #55 — reubicada entre #46 y #48. (3) Regla #61 duplicada (secretos v40.0.0 #138 y traduccion batch v40.0.0 #139 usaban mismo numero) — la primera mantiene #61, la segunda renombrada #61b. Secuencia verificada: 1→96 sin gaps. |
| 2026-03-03 | **59.0.0** | **Sistema de Validacion Arquitectonica Automatizada — 6 Scripts + 4 Puntos de Control Workflow:** 3 patrones nuevos: (1) Validacion cruzada YAML↔PHP (6 scripts en `scripts/validation/` con auto-descubrimiento via glob — `validate-services-di.php` cruza services.yml contra constructor type hints con mapping 30+ servicios core, `validate-routing.php` resuelve PSR-4 y verifica existencia de controller/form/callback con filtro `isCustomClass()` para namespaces jaraba_*/ecosistema_* evitando 72 falsos positivos de core/contrib, `validate-entity-integrity.php` ejecuta 6 checks ENTITY-001/AUDIT-CONS-001/ENTITY-PREPROCESS-001/FIELD-UI-SETTINGS-TAB-001/VIEWS-DATA-001/ECA-EVENT-001, `validate-query-chains.php` detecta patron ->addExpression()->execute() via regex multilinea, `validate-config-sync.sh` detecta config drift con skip de temas, `validate-all.sh` orquesta con --fast <3s y --full), (2) Integracion en 4 puntos de control (pre-commit hook condicional por tipo de archivo staged, CI step antes de PHPStan, deploy step pre-rsync, lando tooling validate/validate-fast), (3) Salvaguarda ECA-EVENT-001 como respuesta a bugs runtime (3 plugins @EcaEvent sin event_name en anotacion generaban warnings — check automatizado añadido al validador de entidades para prevenir recurrencia). Reglas de oro #94 (ARCH-VALIDATE-001 validacion cruzada inter-archivo), #95 (ECA-EVENT-001 event_name obligatorio en anotacion), #96 (BASELINE-CLEAN-001 corregir violations antes de integrar en CI). Aprendizaje #156. |
| 2026-02-28 | **58.0.0** | **Elevacion Sistematica Landing Pages + Homepage — Patron 3 Niveles + METRICS-HONESTY Workflow:** 2 patrones nuevos: (1) Landing elevation sistematica en 3 niveles (Nivel 1: contenido landing — auditar infraestructura real del codebase y reflejar en features/FAQs/stats/testimonials con URLs via Url::fromRoute(); para homepage editar Twig partials directamente ya que es template-driven con fallback data en templates, NO controller-driven; Nivel 2: Schema.org SoftwareApplication JSON-LD con featureList completo + meta description + OG tags + Twitter cards en PageAttachmentsHooks; homepage detectada via isFrontPage() no route name; Nivel 3: config layer FreemiumVerticalLimit + SaasPlan + PlanFeatures coherentes — Talento hereda de Empleabilidad porque es persona recruiter no vertical canonico), (2) METRICS-HONESTY para stats y metricas (SOLO datos verificables desde el codebase o documentacion del programa — 10 verticales verificable via VERTICAL-CANONICAL-001, 11 agentes IA Gen 2, 80+ modulos custom; NUNCA fabricar numeros de usuarios/clientes/conversiones; directiva existente en _stats.html.twig del homepage). Reglas de oro #92 (landing elevation 3 niveles: contenido reflejando infraestructura real + Schema.org en PageAttachmentsHooks + config layer FVL/SaasPlan; homepage es template-driven con Twig partials, deteccion via isFrontPage()), #93 (METRICS-HONESTY: stats y metricas SOLO datos verificables del codebase, NUNCA fabricar numeros). Aprendizaje #155. |
| 2026-02-28 | **57.0.0** | **Claude Code Development Pipeline — 5 Capas DX + 30 Tests Funcionales Workflow:** 4 patrones nuevos: (1) PreToolUse hook para proteccion de archivos (pre-tool-use.sh lee JSON stdin via python3, extrae `tool_input.file_path` directo — NUNCA navegar dentro de `content` que es string en Write tool causando AttributeError; archivos bloqueados con JSON `permissionDecision:deny` en **stdout** + exit 2 — NUNCA enviar a stderr porque Claude Code parsea stdout para el motivo; archivos advertidos con `permissionDecision:ask` + exit 0; archivos normales exit 0 sin output; `CLAUDE_PROJECT_DIR` para ruta relativa), (2) PostToolUse lint por tipo de archivo (post-edit-lint.sh con case/esac por extension: PHP valida syntax `php -l` + detecta `extends ContentEntityForm` violando PREMIUM-FORMS + detecta eval/exec/shell_exec; SCSS detecta hex sin `var(--ej-*)` + @import deprecado + rgba(#hex) sin color-mix; JS detecta fetch con URL hardcoded `/es/` o `/api/` violando ROUTE-LANGPREFIX-001 + innerHTML sin checkPlain; Twig detecta |raw + textos espanol sin trans; errores a **stdout** con exit 2 para feedback directo al agente), (3) Subagentes especializados con context:fork (.claude/agents/{name}/agent.md con YAML frontmatter: name/description/model/context:fork/permissions; reviewer con 4 niveles severidad BLOQUEANTE/WARNING/ADVISORY referenciando 20+ directrices reales; tester con 7 reglas criticas PHP 8.4 MOCK-DYNPROP-001/MOCK-METHOD-001/TEST-CACHE-001/KERNEL-TEST-DEPS-001/KERNEL-TEST-001/KERNEL-SYNTH-001/KERNEL-TIME-001 + templates Unit+Kernel; security-auditor con 10 categorias secrets/XSS/SQLi/CSRF/tenant/access/webhooks/PII/deps/headers), (4) Slash commands con workflow completo (.claude/commands/{name}.md con YAML frontmatter description/argument-hint/allowed-tools; /create-entity genera 9 archivos Entity+Form+Access+ListBuilder+SettingsForm+routing+menu+task+preprocess cumpliendo 10+ directrices automaticamente; /verify-runtime ejecuta 12 checks RUNTIME-VERIFY-001; /audit-wcag ejecuta 10 categorias WCAG 2.1 AA adaptadas al proyecto con colores marca; /fix-issue workflow 8 pasos desde issue hasta PR). Regla de oro #91 (hooks Claude Code: file_path directo de tool_input, deny JSON en stdout no stderr, lint < 5s por archivo, subagentes con directrices reales del proyecto). Aprendizaje #154. |
| 2026-02-28 | **56.0.0** | **Sistema de Coherencia Juridica LCIS 9 Capas — Legal Coherence Intelligence System Workflow:** 5 patrones nuevos: (1) Arquitectura por capas con fail-open (9 servicios PHP puros organizados en capas L1-L8 + MT, cada capa independiente con fail-open — si L2 IntentClassifier falla se asume LEGAL_DIRECT, si L3 NormativeGraphEnricher falla se retornan chunks sin enriquecer, si L6 Validator falla la respuesta pasa sin bloquear; servicios opcionales con `@?` DI y try-catch; capas L1-L4 son PHP puro sin dependencias Drupal para facilitar unit testing con NullLogger), (2) Knowledge Base normativa como SSOT (LegalCoherenceKnowledgeBase con constantes estaticas NORMATIVE_HIERARCHY 9 niveles, NORM_TYPE_PATTERNS 30+ regex, FORAL_LAW_REGIMES 6 CCAA, STATE_EXCLUSIVE_COMPETENCES — toda logica de jerarquia y deteccion de normas centralizada aqui, NUNCA duplicar regex o niveles en otros servicios; metodos estaticos puros sin estado para composicion en cualquier capa), (3) Authority-Aware RAG Ranking formula (NormativeGraphEnricher pesa chunks RAG con formula: final_score = 0.55*semantic + 0.30*authority + 0.15*recency, donde authority viene de getHierarchyWeight() en KnowledgeBase 0.98-0.20, recency es bonus logaritmico por anio, derogation filter elimina normas derogadas, territory warning marca normas foralistas fuera de contexto, CCAA competence bonus +0.15 para normas de la CCAA del usuario), (4) Validator con regeneracion y anti-sycophancy (LegalCoherenceValidatorService ejecuta 7 checks post-LLM con PENALTIES por severidad: hierarchy_inversion critical=-0.30, eu_primacy critical=-0.30, organic_law high=-0.20, competence high=-0.20, vigencia low=-0.05, contradiction medium=-0.15, sycophancy medium=-0.15; score <0.5 + retry_count<2 → action=regenerate con regeneration_constraints CRITICO; score <0.5 + retry_count>=2 → action=block con sanitized_output generico; anti-sycophancy V8 detecta cuando output repite premisa falsa del usuario sin corregir: busca ASSERTION_PATTERNS en user_query y verifica que output contiene CORRECTION_INDICATORS como 'sin embargo'/'conviene aclarar'/'no es exactamente asi'), (5) Multi-turn coherence tracking (LegalConversationContext extrae assertions juridicas por turno con patron position: competencia/primacia/vigencia/reserva_lo/retroactividad/jerarquia, almacena hasta MAX_ASSERTIONS=50, checkCrossTurnCoherence() compara nuevo output contra assertions previas buscando contradicciones semanticas por tipo, MAX_TURNS=20 con trim de turnos antiguos, reset() para nueva conversacion). Regla de oro #90 (LCIS: 9 capas fail-open, KB como SSOT normativa, authority-aware RAG formula, validator con regeneracion y anti-sycophancy, multi-turn coherence tracking). Aprendizaje #153. |
| 2026-02-28 | **55.0.0** | **Unificacion Landing JarabaLex + Despachos — 301 Redirect + FreemiumVerticalLimit + ControllerBase Readonly Fix Workflow:** 3 patrones nuevos: (1) Unificacion de landings verticales via 301 redirect (cuando un vertical tiene multiples landings con features solapadas, unificar en la landing principal con 301 via `Url::fromRoute()` — NUNCA hardcodear paths por ROUTE-LANGPREFIX-001; actualizar TODAS las referencias: megamenu, redirects relacionados como legalRedirect(), SEO meta, Schema.org; mover features advertisadas de la landing absorbida a la principal; verificar que $vertical_routes y $landing_routes en theme incluyen ambas rutas), (2) PHP 8.4 + Drupal 11 ControllerBase readonly property conflict (ControllerBase::$entityTypeManager tiene declaracion `protected` sin tipo en Drupal 11 — los subclasses que usan constructor promotion con `protected readonly EntityTypeManagerInterface $entityTypeManager` causan `Cannot redeclare non-readonly property as readonly`; solucion: quitar la propiedad del constructor promotion y asignar manualmente en el body: `$this->entityTypeManager = $entityTypeManager;` — aplicar a TODOS los controllers que inyecten entityTypeManager), (3) FreemiumVerticalLimit batch creation (patron ID `{vertical}_{plan}_{feature_key}`, 6 feature_keys × 3 planes = 18 configs, valor -1 = ilimitado, valor 0 = bloqueado; sincronizar con FEATURE_TRIGGER_MAP en FeatureGateService + TRIGGER_TYPES en UpgradeTriggerService + getTitleAndMessage() + getIconForType() + buildTriggerResponse() — los 4 puntos deben actualizarse en sync). Reglas de oro #88 (readonly property conflict: NUNCA usar constructor promotion con readonly para propiedades heredadas de ControllerBase), #89 (unificacion de landings: 301 redirect + actualizar todas las referencias megamenu/SEO/theme en un cambio coordinado). Aprendizaje #152. |
| 2026-02-28 | **54.0.0** | **Centro de Ayuda Clase Mundial — Seed Data Platform-Wide + Busqueda Unificada Cross-Entity Workflow:** 3 patrones nuevos: (1) Seed data platform-wide via update hooks (update_10003 crea 25 FAQs con tenant_id omitido — en entity_reference con target_type='group', target_id=0 almacena NULL en BD, por lo que idempotencia requiere `->notExists('tenant_id')` en vez de `->condition('tenant_id', 0)`; migracion de categorias via SQL directo sobre `_field_data` con mapeo array asociativo old→new; helper function `_module_seed_faqs()` retorna array de datos), (2) Busqueda unificada cross-entity type (searchApi() consulta TenantFaq LIKE + KbArticle LIKE con `hasDefinition('kb_article')` guard, todas las URLs generadas con `Url::fromRoute()` — JS NUNCA construye URLs de fallback, campo `type` distingue FAQ de KB en autocompletado, KB usa slug-based URLs `['slug' => $slug]`), (3) Auditoria RUNTIME-VERIFY-001 multi-capa (11 gaps encontrados post-implementacion: CSS faltante para clase JS-generated, URLs hardcodeadas violando LANGPREFIX-001, KB URLs con entity ID en vez de slug, animaciones CSS `opacity:0` sin fallback no-JS, entity_reference target_id=0 → NULL, categorias desordenadas — cada gap entre "codigo existe" y "usuario experimenta" requiere verificar PHP→Twig→SCSS→JS→compiled CSS). Reglas de oro #86 (entity_reference target_id=0 = NULL en BD, usar notExists() para idempotencia), #87 (busqueda unificada cross-entity con hasDefinition + Url::fromRoute + JS sin fallback URLs). Aprendizaje #151. |
| 2026-02-27 | **53.0.0** | **Elevacion UX Clase Mundial 8 Fases — Skeleton + Empty States + Micro-interacciones + Error Recovery + Bottom Nav + Notificaciones + Busqueda Global + Onboarding Quick-Start Workflow:** 8 fases implementadas: (1) Skeleton screens 4 variantes (card/stat/table/list) con shimmer animation y `role="status"`, (2) Empty states con CTA contextual + `jaraba_icon()` duotone, (3) Micro-interacciones CSS (card hover lift, button active scale, modal fadeInScale + `prefers-reduced-motion`), (4) Error recovery con `Drupal.jarabaFetch()` retry exponencial + toast system 4 variantes + CSRF cache como Promise, (5) Bottom nav mobile reutilizando 127 lineas CSS existentes de `_mobile-components.scss` + FAB central con `CustomEvent('jaraba:quick-create')` + body class `has-bottom-nav` via `hook_preprocess_html()`, (6) Centro de notificaciones como modulo `jaraba_notifications` con ContentEntity Notification (EntityOwnerTrait + tenant_id) + 5 endpoints REST + panel glassmorphism, (7) Busqueda global mejorada extendiendo `EntitySearchCommandProvider` de 1 a 3 entity types + relevancia scoring (exact=95, starts_with=75, contains=55) + seccion Recientes via localStorage, (8) Onboarding quick-start overlay con 3 acciones por vertical + focus trap + Esc close + localStorage dismiss. Auditoria post-implementacion: 6 violaciones encontradas y corregidas (2x ICON-COLOR-001, 4x DESIGN-TOKEN-FEDERATED-001 en `_notification-panel.scss`), 12 areas verificadas compliant, 3 tokens CSS nuevos (`--ej-text-on-primary`, `--ej-color-primary-subtle`, `--ej-border-radius-pill`). 16 ficheros creados, 10 modificados, 63+ clases BEM nuevas. Directriz nueva CSS-VAR-ALL-COLORS-001 (P0) en Directrices v100.0.0. Reglas de oro #84 (auditar valores SCSS contra var(--ej-*)), #85 (jaraba_icon() siempre con color paleta). Aprendizaje #150. |
| 2026-02-27 | **52.0.0** | **Navegacion Ecosistema + Pricing Labels (cont.)**: Reglas de oro #82 (opt-in explicito Twig default(false)), #83 (machine names → labels con _raw) incorporadas al listado numerado. |
| 2026-02-27 | **51.0.0** | **Navegación Ecosistema + Pricing Labels — Megamenu Control + Feature Label Display Workflow:** 3 patrones nuevos: (1) Megamenu condicional via preprocess (variables Twig de control de features usan `|default(false)` para ser opt-in explícitas, inyección desde `preprocess_page()` con lógica `empty($variables['meta_site'])` → SaaS principal activa megamenu, meta-sitios conservan nav_items — NUNCA usar `default(true)` para features que dependen de configuración externa), (2) Pricing dual storage machine→label (`formatFeatureLabels()` private method con mapa de 28 machine names → `$this->t('Label')`, aplicado tanto a `features` en `getPricingPreview()` como a `features_highlights` en `getFromPrice()`, `features_raw` conserva machine names para `hasFeature()` y logic checks — NUNCA exponer machine names al usuario), (3) Fallback CSS con variables no definidas (cuando una variable CSS como `--header-bg` no se define en el contexto, el fallback DEBE ser explícito: `var(--header-bg, #ffffff)` — sin fallback el valor es `initial` y causa transparencia). Reglas de oro #82 (opt-in explícito via Twig default(false) + PHP injection), #83 (machine names → labels con campo _raw). Aprendizaje #149. |
| 2026-02-27 | **50.0.0** | **Capa Experiencial IA Nivel 5 — Self-Healing + Autonomous Tasks + Route CSS Bundles Workflow:** 4 patrones nuevos: (1) Self-healing via State API (acciones de auto-remediacion como tier downgrade/provider rotation/throttling usan `\Drupal::state()->set()` con flags efimeros que incluyen campo `expires` para auto-expiry — NUNCA modificar config persistente, State API es efimero y no requiere config:import; patron consumer: `getTierOverride()`/`isProviderRotated()`/`getThrottleConfig()` que verifican expiry y auto-limpian), (2) Autonomous task entity queries (los task methods de agentes autonomos DEBEN consultar entidades reales via EntityTypeManager con `getQuery()->accessCheck(FALSE)` + filtros temporales: ContentCurator busca articulos low-views + gaps publicacion 7d + top performers; KBMaintainer busca knowledge_config 90d stale con `hasDefinition()` para entity types opcionales; ChurnPrevention busca users con 30-60d sin access — NUNCA retornar arrays vacios como stub), (3) Route-specific CSS dashboard pattern (cada dashboard IA tiene su propio SCSS en `scss/routes/{name}.scss` compilado a `css/routes/{name}.css`, declarado como library `route-{name}` en .libraries.yml, mapeado en `hook_page_attachments_alter()` como ruta exacta ANTES del catch-all prefijo modulo, body class via `hook_preprocess_html()` como ruta exacta ANTES del catch-all), (4) SCSS entry point sin partial (cuando el entry point y el partial estan en el mismo directorio `scss/routes/`, Dart Sass `@use` genera ambiguedad entre `name.scss` y `_name.scss` — solucion: consolidar todo el CSS en el entry point sin partial separado, a diferencia del patron con partials en directorios padre). Regla de oro #79 (self-healing efimero via State API, NUNCA config persistente). Aprendizaje #147. |
| 2026-02-27 | **49.0.0** | **Demo Vertical 100% Clase Mundial — Runtime Verification Workflow:** 2 patrones nuevos: (1) SCSS-COMPILE-VERIFY-001 — tras editar CUALQUIER .scss, SIEMPRE recompilar CSS y verificar timestamps con `ls -la`. El gap mas comun entre "codigo existe" y "usuario experimenta" es SCSS editado sin recompilar. Descubierto: _demo.scss 4h+ mas reciente que ecosistema-jaraba-core.css → paginas con estilos rotos. (2) RUNTIME-VERIFY-001 — verificacion post-implementacion de 5 dependencias runtime: CSS compilado, tablas DB hook_schema, rutas .routing.yml, selectores data-* coincidentes template↔JS, drupalSettings controller↔JS. Descubierto: guided tour paso 4 apuntaba a `[data-tour-step="demo-convert"]` inexistente en template. Workflow demo implementado en 4 sprints: S1 tests+quick wins (24 tests, 84 assertions, detach, modal partial), S2 code quality (config wiring, fetch API real, EventSubscriber, partials DRY), S3 architecture (race condition fix, PremiumEntityFormBase migration, preprocess), S4 polish+runtime (guided tour JS driver 290 LOC self-contained, CSS recompilado +18K, constructor bug fix). Reglas de oro #77 (SCSS compilado verificado), #78 (verificacion runtime end-to-end). Aprendizaje #146. |
...
| 2026-02-27 | **48.0.0** | **jaraba_support Clase Mundial — Soporte Omnicanal + SLA + AI + SSE Workflow:** 8 patrones nuevos: (1) State machine con transiciones validadas (VALID_TRANSITIONS mapa estatico, `transitionStatus()` con verificacion de transicion valida + campo changed auto-actualizado, first_responded_at tracking en addMessage cuando agent responde a ticket sin respuesta previa), (2) SLA engine con business hours (SlaPolicy ConfigEntity con ID {plan_tier}_{priority} como lookup key, `calculateDeadline()` delega a BusinessHoursService::addBusinessHours que camina minuto a minuto respetando schedule/timezone/holidays, `processSlaCron()` detecta breached/warned + notifica, pause/resume extiende deadline proporcionalmente), (3) Attachment security multi-capa (upload con MIME whitelist 13 tipos + extension whitelist 14 + double-extension check contra 16 extensiones peligrosas + max size 10MB, ClamAV nSCAN via unix socket con 4-layer heuristic fallback: dangerous ext → double ext → suspicious content 8KB → MIME mismatch, HMAC SHA-256 signed URLs con base64 JSON token + 1h expiry + 3-tier authorization admin/reporter/assignee), (4) Ticket routing multi-factor (agent scoring: skills match +50, vertical match +30, workload inversely proportional +20 max, experience bonus +15 para critical/high, `getNextUrgentTicket()` con priority-weighted sort), (5) SSE via database event queue (TicketStreamService persiste eventos en support_ticket_events, getEventsForAgent filtra por agent assigned tickets + watched tickets con dedup, collision detection via support_ticket_viewers con heartbeat merge + 5min staleness), (6) CSAT survey workflow (CsatSurveyService::scheduleSurvey 1h delay, submitSurvey 1-5 clamp con low satisfaction warning, SupportCronService::sendScheduledSurveys ejecuta pendientes), (7) Health score compuesto (SupportHealthScoreService 5 componentes: volume_trend weight 15, sla_compliance weight 30, csat_average weight 25, escalation_rate weight 15, resolution_speed weight 15, churn alert si score <40), (8) Ticket merge con transferencia integral (TicketMergeService transfiere messages + attachments + watchers con duplicate detection por uid, system message en ambos tickets, event stream logging con metadata source/target). Regla de oro #70 (implementacion completa de modulo de soporte con SLA + AI + SSE en 10 fases). Aprendizaje #145. |
