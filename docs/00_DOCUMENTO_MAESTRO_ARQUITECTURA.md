...
**Fecha:** 2026-03-20
**Versión:** 141.0.0 (PRICING-4TIER-001 + modelo 4 tiers + 8 verticales repriced + 9 addons + features acumulativas + social proof + winback + PauseSubscription + SCSS-COMPONENT-BUILD-001 + 72 scripts + aprendizaje #206 + regla de oro #144)
...
| 2026-03-18 | **132.0.0** | **Kit Digital Agente Digitalizador — 4 Fases Completas:** jaraba_billing: KitDigitalAgreement entity (17 campos, lifecycle 7 estados draft→paid), KitDigitalService (10 métodos, tabla bonos paquete×segmento Anexo IV), KitDigitalController (landing + 5 paquetes Anexo II Red.es 9 requisitos), KitDigitalAdminController (dashboard KPIs + distribución), cron processExpiredAgreements(), 3 emails, SetupWizard step + 2 Daily Actions. Zero-region templates, SCSS mobile-first 420 líneas. 4 logos SVG. PHPStan baseline 46141. 12 tests, 112 assertions. 7 reglas KIT-DIGITAL-001 a 007. Regla de oro #135. Aprendizaje #194. |
| 2026-03-18 | **129.0.0** | **Safeguard System 27 Scripts + Pre-commit Activation + lint-staged DI Validators:** Sistema salvaguarda actualizado: 27 scripts validación (de 24), pre-commit hook chmod +x activado (git ignoraba silenciosamente), lint-staged services.yml→4 validators DI específicos (phantom-args, optional-deps, circular-deps, logger-injection). validate-phantom-args.php v2: detección bidireccional + fix parser inline comments + 12 tests regresión. 3 servicios DI corregidos (analytics_service +@database+@logger.factory, stripe_invoice +@http_client, quote_estimator +@http_client). 68 AccessControlHandlers AccessResult→AccessResultInterface. Drupal core + contrib 773 archivos. Aprendizaje #191. |
