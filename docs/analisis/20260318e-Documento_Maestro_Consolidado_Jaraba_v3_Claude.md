...
# Version: 3.0 | Fecha: 18 marzo 2026 | Estado: Production-ready, fase aceleracion PLG
# Auditado contra codebase: 18 correcciones aplicadas (Claude Code audit v1)
# Operado por: Plataforma de Ecosistemas Digitales S.L.
# Equipo dev: Claude Code (integramente)
...
**Nota (v3):** Doc 158 SSOT lista 9 Marketing Add-ons. La referencia "8" en versiones anteriores era erronea. Cifra canonica: **9**.
...
**Datos verificados de expedientes (resoluciones oficiales SAE):**

| Expediente | Importe | Proyectos | Provincias | Periodo |
|-----------|---------|-----------|------------|---------|
| SC/ICV/0156/2023 (Resolucion 15/12/2023) | 225.000 EUR | 50 | 8 (toda Andalucia) | 26/12/2023 — 25/06/2025 |
| SC/ICV/0111/2025 (Resolucion 19/12/2025) | 202.500 EUR | 45 | 2 (Malaga + Sevilla) | 29/12/2025 — 28/06/2027 |
| **TOTAL** | **427.500 EUR** | **95** | | |

Anticipos cobrados: 180.000 EUR (2023) + 162.000 EUR (2025) = 342.000 EUR. Financiacion: FSE+ 85% + Junta Andalucia 15%.

**NOTA v3:** El expediente SC/ICJ/0050/2024 referenciado en versiones anteriores NO EXISTE. Los datos de "640 participantes" y "900K EUR" eran erroneos. Los datos correctos son 95 participantes y 427.500 EUR, verificados contra resoluciones oficiales firmadas por Antonio Tallante Ruiz, Director General de Intermediacion y Orientacion Laboral del SAE.
...
|----|-----|-----------|--------|
| GAP-01 | Integracion bidireccional STO (sync participantes, acciones, fases) | CRITICA | **Implementado** (StoExportService.php) |
...
| GAP-03 | Gestion transicion Fase Atencion → Fase Insercion segun normativa | CRITICA | **Implementado** (FaseTransitionManager.php, 6 fases canonicas) |
| GAP-04 | Entidad programa_participante_ei con campos STO | ALTA | **Implementado** (ProgramaParticipanteEi.php ContentEntity) |
| GAP-05 | Computo y validacion 25h de Mentoria IA (Tutor Jaraba) | ALTA | **Implementado** (AiMentorshipTracker.php, campo horas_mentoria_ia) |
| GAP-06 | Sistema de Recibi del incentivo economico (528 EUR) | ALTA | Parcial (campo incentivo_recibido existe en entity) |
| GAP-07 | Modulo jaraba_learning especifico para programa 100h | MEDIA | Parcial |
...
| LMS con tracking xAPI + H5P | ✓ Cumple (jaraba_lms: XApiService + XapiController + H5P content) |
| Videoconferencia integrada | ✓ Cumple (Jitsi Meet) |
| Emision de certificados | ✓ Cumple (Open Badges 3.0) |
| Web Service SOAP SEPE | ✓ **Implementado** (SepeSoapService.php: 6 operaciones Anexo V + SepeDataMapper + SepeSoapController) |
...
**CAPA 6 — Infraestructura:** IONOS Dedicated L-16 NVMe (AMD EPYC 4465P, 128GB DDR5, 2x1TB NVMe). MariaDB 10.11 (InnoDB 16G). Redis 7.4. Qdrant Cloud (~25 USD/mes en produccion; Docker self-hosted en dev local). Apache Tika (Docker stateless). Nginx (reverse proxy + SSL via IONOS). **Produccion usa Docker Compose con estrategia blue-green** (deploy-production.yml: Docker Buildx + `docker compose up -d web-$DEPLOY_TO`; Nginx hace upstream switch entre contenedores web-blue/web-green). Servicios de datos (MariaDB, Redis) pueden ser nativos. WAF: ModSecurity + OWASP CRS + Fail2ban + Nginx rate limiting. Backup: Hetzner Object Storage + NAS 16TB (GoodSync SFTP pull daily).

**CAPA 5 — DevOps:** GitHub Actions: 8 workflows, 2.803 LOC (ci, security-scan, deploy x3, backup x2, fitness). Blue-green deployment con rollback automatico. 27 scripts de validacion. Lando (.lando.yml) con 7 servicios (appserver, database, redis, qdrant, tika, phpmyadmin, mailhog) + 5 subdominios locales.
...
**Nota version PHP:** Produccion = PHP 8.4. Algunos docs spec tempranos y CI (132_CICD) referencian 8.3. Stack mandatorio es 8.4. MariaDB: produccion y CI usan ambos 10.11 (alineados).
...
**FOC (Financial Operations Center):** jaraba_foc (1 modulo, 32 archivos PHP; las funcionalidades de entities, ETL, metrics y forecasting estan dentro del modulo como servicios, NO como submodulos independientes). SSOT centralizado en Drupal. Metricas SaaS: MRR, ARR, Churn, LTV, CAC. Proyecciones PHP-ML con escenarios. Sistema de alertas prescriptivas con playbooks ECA automatizados (Churn Prevention, Revenue Acceleration). Cost Allocation para multi-tenancy a nivel atomico (tenant individual + producto).
...
- Webhook auto-provisioning (checkout.session.completed) — ALTA
- Stripe Customer Portal integracion — ALTA
- Dunning (cobro fallido: invoice.payment_failed → email + grace period + suspension) — ALTA
...
Colores: azul-corporativo #233D63, naranja-impulso #FF8C42, verde-innovacion #00A9A5. 35+ CSS custom properties --ej-* inyectadas en runtime (configurables por tenant via hook_preprocess_html), mas 100+ variables SCSS de compilacion. 15 industry presets. Usar color-mix() (no rgba()). 5 niveles de design tokens.
...
1. Dependency Audit: Composer + npm (high/critical = blocking). 2. Trivy: Filesystem + secrets (CRITICAL/HIGH = blocking). 3. SAST: PHPStan L6 + phpstan-security.neon. 4. DAST: OWASP ZAP baseline. 5. Runtime: hook_requirements() en 83/94 modulos. 6. PII: Guardrails bidireccionales (DNI, NIE, IBAN ES, NIF/CIF, +34).
...
## 16. CORRECCIONES v3: OMISIONES INCORPORADAS (Auditoria Claude Code)

### O1 — STRIPE_WEBHOOK_SECRET
Variable critica para verificacion HMAC de webhooks (AUDIT-SEC-001). Debe estar en `settings.secrets.php` junto con STRIPE_SECRET_KEY y STRIPE_PUBLISHABLE_KEY. Sin ella, `checkout.session.completed` e `invoice.payment_failed` no se verifican.

### O2 — PHANTOM-ARG-001 v2 (bidireccional)
Evolucion de la regla original: deteccion bidireccional de argumentos de MAS (phantom) Y de MENOS (missing). Missing es mas peligroso que phantom (`$container->has()` devuelve TRUE pero `get()` lanza TypeError transitivo). 12 tests de regresion en `tests/test-phantom-args-parser.php`. 13 servicios descubiertos con args faltantes en v2.

### O3 — CSRF-LOGIN-FIX-001 v2
Fix critico de produccion: IONOS termina SSL; Apache/PHP recibe HTTP. `SessionConfiguration.php:55` overrides `cookie_secure` con `$request->isSecure()`. Si isSecure() es inconsistente → session perdida → CSRF falla. Fix: `$_SERVER['HTTPS']='on'` desde X-Forwarded-Proto ANTES del bootstrap Drupal. `patch-settings-csrf.php` se ejecuta en cada deploy.

### O4 — Copilot Bridges (aprendizaje #183)
Bridge services que conectan copilots verticales con el orquestador: `DemoCopilotBridgeService` (vertical demo), `LegalCopilotBridgeService` (JarabaLex). Reglas: COPILOT-BRIDGE-COVERAGE-001 y STREAMING-PARITY-001.

### O5 — ZEIGARNIK-PRELOAD-001
Patron psicologico clave: 2 auto-complete global steps (`__global__` wizard ID) inyectados en TODOS los wizards. Los wizards arrancan al 25-33% de completion (efecto Zeigarnik: +12-28% tasa de finalizacion). Implementado via tagged services con wizard_id = `__global__`.

### O6 — ACCESS-RETURN-TYPE-001
Regla PHPStan: `checkAccess()` DEBE declarar `: AccessResultInterface` (NO `: AccessResult`). `parent::checkAccess()` devuelve `AccessResultInterface`; return type mas restrictivo causa error PHPStan. 68 AccessControlHandlers migrados en commit `f09dbaa95`.

### O7 — SAFEGUARD-CANVAS-001
Proteccion de 4 capas para el canvas GrapesJS: (1) Backup automatico pre-save (canvas_data snapshot), (2) Restore point via revision history, (3) Presave validation (JSON schema check), (4) Post-save verification (rendered output matches).

---

*Documento v3 generado 18 marzo 2026. Auditado contra codebase por Claude Code (94 modulos, 1.131 servicios, 653 commits, CLAUDE.md v1.5.4, 18 memory files). 18 correcciones aplicadas: 7 errores factuales, 4 imprecisiones, 7 omisiones.*
*Fuentes: Revision Profunda Estado SaaS v1 (Claude Code 1M context), project knowledge, memory del proyecto, documento addons/verticales canonicos.*
