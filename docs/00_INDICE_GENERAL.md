...
**Versión:** 207.0.0 (VIEWPORT-DVH-001 + NOWRAP-OVERFLOW-001 + IMG-SEED-001 — mobile-first 10/10, 201 validators, aprendizaje #237)

> **📋 WhatsApp-First Email Protection — v168 DIRECTRICES + v151 ARQUITECTURA + v202 INDICE** (2026-03-27)
>
> Aprendizaje #227: AI-COVERAGE-001 — Cobertura IA autonoma completa del SaaS. Auditoria identifica score 6.2/10: infraestructura Gen 2 excelente (8 agentes, model routing, 5 Supervisor workers) pero 6 modulos operativos sin integracion IA (CRM, Billing, Support, Email, Social, Analytics) y 5 servicios predictivos ML aislados en jaraba_predictive sin conexion a consumidores. 4 sprints implementados: (1) CRM: CrmCopilotBridgeService (pipeline forecast, BANT, top opportunities) + OpportunityGroundingProvider (prioridad 60) + ContactGroundingProvider (55), (2) Billing: BillingCopilotBridgeService (MRR/ARR/churn) + InvoiceGroundingProvider (45), (3) Support: SupportCopilotBridgeService (tickets SLA) + SupportTicketGroundingProvider (knowledge base resueltos), (4) Email/Social/Analytics: 3 CopilotBridge + 2 GroundingProvider. PredictiveIntegrationService en ecosistema_jaraba_core como bridge pattern centralizado (5 metodos: getLeadEnrichment, getChurnRisk, getRevenueForecast, detectAnomalies, triggerRetention). AutonomousAgentService expandido de 4 a 10 tipos sesion (+crm_intelligence, revenue_optimization, content_seo_optimizer, support_proactive, email_optimizer, social_optimizer). Dashboard wiring: CRM y Billing controllers inyectan datos predictivos via PredictiveIntegrationService. Cron retention: hook_cron cada 6h itera high-risk tenants (score >= 60) y dispara RetentionWorkflowService. Nuevo validador validate-ai-coverage.php (100% score). 17 tests unitarios, 47 assertions. Bridge Registry: 16 bridges (10 verticales + 6 operativos) con @? (OPTIONAL-CROSSMODULE-001). GroundingProvider: 17 providers via CompilerPass (tag jaraba_copilot_v2.grounding_provider). Score final: 9.5/10 (falta solo runtime verification en produccion y actualizacion baseline PHPStan). Regla de oro #161: La paradoja del modulo predictivo — tener servicios ML implementados pero aislados es peor que no tenerlos, porque genera falsa confianza. La diferencia entre "el codigo existe" y "el usuario lo experimenta" requiere verificar PIPELINE-E2E-001 en CADA capa: Service → Controller → hook_theme → Template.
...
>
> Aprendizaje #210: Activación IA del Embudo de Ventas — Visitante anónimo pregunta "busco curso con incentivo" al copilot público y no recibe información sobre el Programa Andalucía +ei en promoción activa. Causa raíz: (1) Prompt estático hardcodeado con 4 demos de 10 verticales, (2) ContentGroundingService buscaba solo en 3 entity types, (3) Sistema de promociones desacoplado del copilot, (4) Copilot no generaba leads CRM. Solución: 7 nuevas reglas: GROUNDING-PROVIDER-001 (10 providers tagged), CASCADE-SEARCH-001 (4 niveles búsqueda progresiva), ACTIVE-PROMOTION-001 (PromotionConfig ConfigEntity + ActivePromotionService), COPILOT-LEAD-CAPTURE-001, COPILOT-FUNNEL-TRACKING-001. ContentGroundingService v2 con GroundingProviderInterface + CompilerPass. Prompt dinámico buildDynamicPublicSystemPrompt(). 3 validadores nuevos. 86 scripts total (91 checks). Regla de oro #147: El copilot público DEBE tener acceso a TODAS las promociones activas y contenido de TODAS las verticales via GroundingProviders — un SaaS de IA nativa NUNCA puede tener su IA desconectada de sus propias ofertas comerciales.
...
>
> Aprendizaje #209: DEDICATED-SERVER-PRODUCTION-001 — Migración completa shared hosting → IONOS Dedicated AE12-128 NVMe (AMD EPYC 4465P, 128GB DDR5, 2x1TB NVMe RAID1). Stack nativo: Nginx 1.24 + PHP-FPM 8.4 (24 workers, JIT) + MariaDB 10.11 (InnoDB 16GB) + Redis 7.4 (5GB) + Supervisor (5 AI workers) + Tika Docker. Cutover DNS sin downtime significativo (~17min). JS-AGGREGATION-LAZY-001: Nginx location para archivos agregados CSS/JS DEBE usar try_files con fallback a index.php (Drupal 11 lazy generation). INFRA-HEALTH-001 ampliado a 13 checks (Redis memory, AI workers, backup freshness). Deploy workflow: sync automático Nginx config con diff + reload condicional. settings.local.php template incluye settings.production.php via dirname(). Deploy limpia CSS/JS agregados obsoletos en cada deploy para forzar regeneración. 4 dominios SSL Let's Encrypt, 18 GitHub Secrets.
>
...
>
> Aprendizaje #187: Stripe Checkout flujo completo operativo. 7 bugs P0-P2 descubiertos y corregidos: (1) CHECKOUT-ROUTE-COLLISION-001 — Commerce Checkout `/checkout/{order}/{step}` con step=null default captura TODOS los `/checkout/*`, rutas billing movidas a `/planes/checkout/*`; (2) STRIPE-URL-PREFIX-001 — `stripeRequest()` endpoints con `/v1/` duplicado (base URL ya incluye `/v1`), afectaba StripeProductSyncService y CheckoutSessionService; (3) CSP-STRIPE-SCRIPT-001 — `js.stripe.com` faltaba en `script-src` de CSP (solo estaba en `frame-src`); (4) 404 sin degradación elegante cuando Stripe no configurado; (5) Metadata mismatch en webhook handler (`plan_tier` no existía en session metadata, resuelto via `drupal_plan_id`→SaasPlan entity); (6) `processRegistration()` incompatible con checkout (requería password/domain — implementado dual-flow usuario existente/nuevo); (7) `catch(\Exception)` en 12 sitios de BillingWebhookController+DunningService (PHP 8.4 TypeError extiende Error, no Exception). Sincronización bidireccional: 17 Products + Prices creados en Stripe con metadata trazable. 9 email templates transaccionales: trial_will_end, payment_failed, subscription_cancelled + 6 dunning steps (email_soft→email_urgent→restrict→suspend→final_notice→cancel). Checkout premium: body limpio sin clases contaminantes (early return en preprocess_html + exclusión jaraba_theming), header 3 zonas, trust bar, Stripe Embedded directo sin formulario pre-Stripe redundante. Regla de oro #128: Stripe API endpoints en `stripeRequest()` NUNCA con prefijo `/v1/`.
...
> **📋 PREMIUM FORMS SLIDE-PANEL UX — v130 DIRECTRICES + v118 ARQUITECTURA** (2026-03-13)
> - **Contexto:** Mejora UX de formularios premium en slide-panel del Coordinador Hub Andalucía +ei. 3 reglas nuevas: CHECKBOX-SCOPE-001 (toggle switch scoped a `.form-type-boolean`, card-style para multi-option lists), OBSERVER-SCROLL-ROOT-001 (IntersectionObserver con `.slide-panel__body` como root), STICKY-NAV-WRAP-001 (nav pills sticky con flex-wrap + backdrop-filter blur). SCSS `_premium-forms.scss` + JS `premium-forms.js` actualizados. Regla de oro #121. Aprendizaje #180.

> **📋 SPRINT 15-18 ANDALUCÍA +ei CLASE MUNDIAL 10/10 — v129 DIRECTRICES + v117 ARQUITECTURA** (2026-03-12)
> - **Contexto:** Sprint 15-18 completo del Plan Maestro Andalucía +ei. **Sprint 15 (9 P0):** restricción provincias FT_679, alineación canTransitToInsercion 4 criterios, reapertura baja, contrato temporal InsercionLaboral, seguimiento ≥40h, STO plazo 15 días, ActuacionComputeService. **Sprint 16 (5 tools):** DIME auto-asignación carril, 4 alertas plazos normativos, BMC dashboard. **Sprint 17 (8 world-class):** StoBidireccionalService (push/pull/reconciliation), ICalExportService RFC 5545 + HMAC auth, EiMultichannelNotificationService (push+WhatsApp+in-app GDPR), advanced analytics (funnel+distributions+burndown), gamification leaderboard, Club Alumni panel, PWA route integration, Copilot restricción por fase (11 modos × 6 fases). **Sprint 18:** WCAG 2.1 AA (47 scope="col", contrast tokens --hub-text-naranja/verde, semantic role="group"), i18n 100%, 432 tests 0 failures (+17 nuevos). 3 servicios nuevos, 3 controllers nuevos, 68 ficheros modificados.
> - **Aprendizaje:** #179 — Implementación clase mundial requiere auditoría integral multidisciplinar (normativa+operacional+WCAG+testing) como punto de partida, no como verificación final. HMAC-SHA256 con hash_salt de Drupal para autenticación sin sesión en feeds iCal. color-mix(in srgb, token 65%, black) para tokens de texto WCAG AA sobre fondos claros.
> - **Cross-refs:** Directrices v129.0.0, Arquitectura v117.0.0, Indice v158.0.0, Flujo v82.0.0.
>
> **📋 SPRINT 14 REESTRUCTURACIÓN ACTUACIONES PIIL — v128 DIRECTRICES + v116 ARQUITECTURA** (2026-03-11)
> - **Contexto:** Sprint 14 completo del Plan Maestro Andalucía +ei. Alineación normativa con BBRR Art. 2 + STO Manual ICV25. 1 ContentEntity nueva: MaterialDidacticoEi (7 tipos, archivo+url_externa). 1 servicio nuevo: ActuacionComputeService (cálculo automático persona atendida ≥10h orient.+≥50h form.+≥75% asist. y persona insertada atendida+≥40h orient.ins.+contrato≥4m/RETA, triggered via InscripcionSesionEi::postSave). 5 entities modificadas: SesionProgramadaEi (6 tipos PIIL + FASE_POR_TIPO + LEGACY_MAP + fase_piil/contenido_sto/subcontenido_sto), AccionFormativaEi (+contenido_sto+subcontenido_sto+materiales), ActuacionSto (8 tipos PIIL + LEGACY_MAP), PlanFormativoEi (+3 campos computed), InscripcionSesionEi (+postSave trigger). CoordinadorHubService ampliado con getEstadisticasPorFase()+getUpcomingSessionsPiil(). Dashboard coordinador: nuevo tab PIIL con bloques Fase Atención/Inserción + KPIs persona atendida/insertada. Migration script migrate_sesion_tipos_piil.php (3 fases). hook_update_10023. 17 tests nuevos (368 total). SCSS PIIL panel compilado.
> - **Aprendizaje:** #178 — Alineación normativa PIIL requiere reestructuración del sistema de tipos (no solo añadir campos). Mapas de migración legacy con constantes en la entity interface permiten migración de datos backward-compatible. Triggers postSave para métricas derivadas DEBEN ser resilientes (hasService+try-catch).
> - **Cross-refs:** Directrices v128.0.0, Arquitectura v116.0.0, Indice v157.0.0, Flujo v81.0.0.
>
...
> - **Cross-refs:** Directrices v126.0.0, Arquitectura v114.0.0, Indice v155.0.0, Flujo v79.0.0.
>
> **📋 ENV-PARITY-001 DEV/PROD PARITY + 18 SCRIPTS VALIDACION — v125 DIRECTRICES + v113 ARQUITECTURA** (2026-03-11)
> - **Contexto:** Nuevo safeguard ENV-PARITY-001 para prevenir bugs que solo aparecen en produccion. Script #18 validate-env-parity.php con 14 checks de paridad dev/prod (PHP version en 6 fuentes, 14 ext-* en composer.json, MariaDB, Redis, PHP config, MariaDB my.cnf, OPcache, Supervisor, filesystem, multi-domain, code paths, composer.lock, reverse proxy, wildcard SSL). Infraestructura produccion: config/deploy/mariadb/my.cnf (InnoDB 16G escalable, NVMe tuning), OPcache invalidation en deploy.yml, 12 ext-* en composer.json, max_input_vars 5000 en php.ini. Analisis runbook servidor AMD EPYC Zen 5 (AE12-128 NVMe, 12c/24t, 128GB DDR5, 2x1TB NVMe). validate-all.sh actualizado con ENV-PARITY-001 + 3 skip_checks faltantes.
> - **Aprendizaje:** #175 — Dev/Prod Parity Gaps, RecursiveDirectoryIterator vs glob(**), FPM-only extensions.
> - **Cross-refs:** Directrices v125.0.0, Arquitectura v113.0.0, Indice v154.0.0, Flujo v78.0.0.
>
> **📋 I18N NAVIGATION + CANVAS SAFEGUARD 4 CAPAS — v124 DIRECTRICES + v112 ARQUITECTURA** (2026-03-10)
> - **Contexto:** Sistema completo de internacionalización de navegación + 4 capas de protección canvas. 5 reglas nuevas: I18N-NAVPREFIX-001 (language_prefix en todos los headers/footer), I18N-PATHPROCESSOR-001 (eliminación filtro langcode en PathProcessor), I18N-LANGSWITCHER-001 (language switcher con path_alias traducido), SAFEGUARD-CANVAS-001 (backup/restore + presave hook + translation safeguards + validation), SAFEGUARD-ALIAS-001 (detección alucinación path_alias IA). 68 double-slash aliases corregidos. 1 alucinación IA corregida. validate-nav-i18n.php nuevo. 5 headers + footer + mobile overlay prefijados.
> - **Aprendizaje:** #174 — I18N Navigation, Canvas Safeguard, PathProcessor langcode fix, Alias hallucination detection.
> - **Cross-refs:** Directrices v124.0.0, Arquitectura v112.0.0, Indice v153.0.0, Flujo v77.0.0.
>
> **📋 ANTI-SPAM + COORDINADOR DASHBOARD CLASE MUNDIAL + RECLUTAMIENTO SEO — v123 DIRECTRICES + v111 ARQUITECTURA** (2026-03-10)
> - **Contexto:** Anti-spam stack consolidation (login zero friction, reCAPTCHA keys via deploy secrets). Coordinador dashboard elevado a clase mundial (19 gaps: empty states, focus trap modales, CSRF TTL 1h, aria-live, detach, responsive tables, reduced-motion, focus-visible). /user profile widgets Andalucía +ei (badges fase, stats row, completitud success). Reclutamiento landing elevation (social proof, lead magnet, 12 FAQs SEO, BreadcrumbList, og:image). cross_vertical fix. 21 test files nuevos → 312 tests / 1122 assertions / 0 errores. 2 reglas nuevas: RECAPTCHA-ENV-INJECT-001, ANTI-SPAM-STACK-001.
> - **Aprendizaje:** #173 — Anti-Spam Stack Differentiation, Coordinador Dashboard Clase Mundial, Reclutamiento Landing Elevation SEO.
> - **Cross-refs:** Directrices v123.0.0, Arquitectura v111.0.0, Indice v152.0.0, Flujo v76.0.0.
>
> **📋 PLAN MAESTRO ANDALUCÍA +ei 12 SPRINTS CLASE MUNDIAL — v122 DIRECTRICES + v110 ARQUITECTURA** (2026-03-10)
> - **Contexto:** 12 sprints para cumplimiento PIIL CV 2025 (Junta de Andalucía + FSE+). Firma electrónica (8 estados, 3 métodos, 37 categorías), PlanEmprendimientoEi (20 campos, 4 fases), 7 bridge services, matching Qdrant, alumni, badges, push notifications. 10 gaps P0 resueltos. 231 tests. SERVICE-CONSUMER-001. Golden rules #108-112. Aprendizaje #172.
> - **Cross-refs:** Directrices v122.0.0, Arquitectura v110.0.0, Indice v151.0.0, Flujo v75.0.0.
>
> **📋 TWIG RENDER ARRAY SAFETY + ENTITY UPDATE HOOK HARDENING — v121 DIRECTRICES + v109 ARQUITECTURA** (2026-03-09)
> - **Contexto:** Auditoria SEO/GEO profunda del meta-sitio corporativo + elevacion landing reclutamiento. 5 reglas nuevas, NAP normalizado, Schema.org enriquecido, orphan templates resueltos, og:image fallback, hreflang dedup, cookie localStorage-first.
> - **Ficheros:** jaraba_page_builder.module (og:image fallback), ecosistema_jaraba_theme.theme (hreflang abs, is_page_content), html.html.twig (schema includes), _ped-schema.html.twig (GeoCoordinates), jaraba_andalucia_ei.module (ZERO-REGION-003 + geo + OG), cookie-banner.js (localStorage-first), reclutamiento-popup.js, map.html.twig (NAP fix).
> - **Aprendizaje:** #170 — Orphan templates, og:image fallback chain, hreflang conditional, NAP consistency, meta-site homepage detection.
>
...
> - **Ficheros nuevos:** StripeProductSyncService, CheckoutSessionService, CheckoutController, stripe-checkout.js, page--checkout.html.twig, checkout-page.html.twig, checkout-success.html.twig, checkout-cancel.html.twig, _checkout.scss, StripeSyncCommands (Drush).
> - **Ficheros modificados:** MetaSitePricingService, pricing-page.html.twig, pricing-toggle.js, ecosistema_jaraba_theme.theme, main.scss, jaraba_billing.module/.services.yml/.routing.yml/.libraries.yml.
> - **1 regla nueva:** STRIPE-CHECKOUT-001 (P0). Regla de oro #107. Aprendizaje #165.
> - **Cross-refs:** Directrices v115.0.0, Arquitectura v104.0.0, Flujo v68.0.0.
...
> - **3 reglas nuevas:** DOMAIN-ROUTE-CACHE-001 (P0), VARY-HOST-001 (P0), PHANTOM-ARG-JS-001 (P1). Reglas de oro #104-105. Aprendizaje #162.
> - **Cross-refs:** Directrices v112.0.0, Arquitectura v101.0.0, Flujo v65.0.0.
> - **Docs nuevos 2026-03-05:** `docs/analisis/2026-03-05_Auditoria_Gaps_Billing_Commerce_Fiscal_v1.md`, `docs/analisis/2026-03-05_Auditoria_Flujo_Onboarding_Checkout_Clase_Mundial_v1.md`, `docs/implementacion/2026-03-05_Plan_Implementacion_Remediacion_Billing_Commerce_Fiscal_Clase_Mundial_v1.md`, `docs/implementacion/2026-03-05_Plan_Implementacion_Remediacion_Onboarding_Checkout_Catalogos_Clase_Mundial_v1.md`, `docs/implementacion/2026-03-05_Plan_Implementacion_Verticales_Componibles_Addon_Marketplace_v1.md`.

> **🔧 VERTICALES COMPONIBLES + TENANT SETTINGS HUB + STRIPE SYNC** (2026-03-05)
> - **Contexto:** Implementacion de 2 features mayores: (1) Sistema de verticales componibles donde tenants activan verticales adicionales como add-ons (addon_type='vertical' + TenantVerticalService + VerticalAddonBillingService + 4 REST endpoints), (2) Tenant Settings Hub unificado con registry pattern + compiler pass + 6 secciones tagged (Domain/Plan/Branding/Design/ApiKeys/Webhooks). Stripe sync bidireccional: changePlan() sync con Stripe, revert local si falla. FeatureAccessService unificado (legacy+nuevo). Theme token override auto-resolve tenant_id. AI agents enriquecidos con verticales addon en contexto.
> - **2 reglas nuevas:** ADDON-VERTICAL-001 (P1), TENANT-SETTINGS-HUB-001 (P1). Reglas de oro #102-103. Aprendizaje #161.
> - **Cross-refs:** Directrices v111.0.0, Arquitectura v100.0.0, Flujo v64.0.0.

> **📊 CI/PHPSTAN FIXES — ACCESS-RETURN-TYPE-001 + PIPELINE VERDE** (2026-03-04)
> - **Contexto:** Sesion de estabilizacion CI/PHPStan. PHPStan Level 6: 1890 errores corregidos en baseline (return type AccessResult→AccessResultInterface en 130+ AccessControlHandlers, entity field access, otros). CI fitness-functions pipeline: 35 errores→0 (PHPStan + PHPCS + Kernel tests). Kernel tests: 460 tests, 2243 assertions, 0 errores. PHPStan baseline reducido a ~41K lineas.
> - **1 regla nueva:** ACCESS-RETURN-TYPE-001 (P0 — checkAccess() return type debe ser AccessResultInterface). Regla de oro #101. Aprendizaje #160.
> - **Cross-refs:** Directrices v110.0.0, Arquitectura v99.0.0, Flujo v63.0.0.

> **📊 STRIPE CONNECT INTEGRATION + DI AUDIT RUNTIME FIXES** (2026-03-04)
> - **Contexto:** Configuracion completa de Stripe Connect como plataforma/marketplace + auditoria exhaustiva de inyeccion de dependencias tras errores runtime en cascada. 4 reglas nuevas: LOGGER-DI-001, CIRCULAR-DEP-001, PHANTOM-ARG-001, STRIPE-ENV-UNIFY-001.
> - **Stripe:** Cuenta tipo plataforma, destination charges, subscriptions + metered billing, 2 webhooks con secrets separados, settings.secrets.php unificado para 3 namespaces config, StripeConnectService con getenv() fallback, Stripe CLI v1.37.2.
> - **DI Audit:** 21 mismatches en 9 modulos (12 logger channel→factory, 6 phantom args, 3 otros). 6 dependencias circulares resueltas. PHP 8.4 nullable fix.
> - **Gaps Salvaguarda:** Sin deteccion de circular deps, sin validacion phantom args, logger mismatch solo en modo --full.
> - **Reglas de oro:** #99 (LOGGER-DI-001 verificar type hint), #100 (args services.yml = params constructor). Aprendizaje #159.
> - **Cross-refs:** Directrices v109.0.0, Arquitectura v98.0.0, Flujo v62.0.0.

> **📊 7 ITEMS SAAS POST-EVALUACION — VerticalBrand + Pre-PMF + PilotManager + PIIL + OpenAPI + Safeguards** (2026-03-04)
> - **Contexto:** Implementacion de 7 items estrategicos post-evaluacion de documentos 20260303. Estrategia "Submarinas con Periscopio" — cada vertical presentado como producto independiente con sub-marca, revelacion progresiva en 4 niveles (landing → trial → expansion → enterprise). Principio rector: extender modulos existentes sobre crear nuevos (unico modulo nuevo: jaraba_pilot_manager).
> - **Item 1 (VerticalBrand):** ConfigEntity VerticalBrandConfig en ecosistema_jaraba_core (18 campos, 10 YAMLs) + RevelationLevelService + NavigationFilterService + VerticalBrandSeoService + VerticalBrandController.
> - **Item 2 (Pre-PMF):** ProductMetricSnapshot ContentEntity (16 campos) + ActivationCriteriaConfig ConfigEntity (10 YAMLs) + 5 Services analytics + PrePmfDashboardController + NPS API en jaraba_analytics.
> - **Item 3 (Twig feature_allowed):** FeatureGateRouterService (ServiceLocator 10 verticales) + TwigTest en JarabaTwigExtension.
> - **Item 4 (PilotManager):** MODULO NUEVO jaraba_pilot_manager — 3 ContentEntities (PilotProgram, PilotTenant, PilotFeedback) + PilotEvaluatorService + PilotDashboardController + 5 ECA Events.
> - **Item 5 (PIIL):** +7 campos ProgramParticipant + 6 campos InstitutionalProgram + PiilMetricsService + StoSyncService + PiilDashboardController en jaraba_institutional.
> - **Item 6 (OpenAPI):** OpenApiSpecService + OpenApiController (/api/v1/openapi.json) en jaraba_integrations.
> - **Item 7 (Safeguards):** CHECK 7 validate-entity-integrity.php, UPDATE-HOOK-REQUIRED-001 reforzado ConfigEntities, 3 hook_update_N pilot_manager.
> - **1 regla reforzada:** UPDATE-HOOK-REQUIRED-001 (ConfigEntities NO exentas). Regla de oro #98. Aprendizaje #158.
> - **Cross-refs:** Directrices v108.0.0, Arquitectura v97.0.0, Flujo v61.0.0.

> **⚖️ LCIS GAP CLOSURE — EU AI Act Audit Trail + Normative Graph** (2026-03-04)
> - **Contexto:** 4 gaps de complitud cerrados entre spec 179 y la implementacion real del LCIS (Legal Coherence Intelligence System). 2 entities nuevas: `legal_norm_relation` (10 tipos de relacion normativa, grafo de derogaciones para NormativeGraphEnricher L3) y `legal_coherence_log` (15 campos audit trail EU AI Act Art. 12, read-only). 2 QueueWorkers: NormativeUpdateWorker (vigencia) y NormVectorizationWorker (Qdrant). Decision arquitectonica: NO crear modulo standalone jaraba_lcis — subsistema distribuido entre jaraba_legal_knowledge (datos) y jaraba_legal_intelligence (pipeline).
> - **2 reglas nuevas:** LCIS-AUDIT-001 (P0), LCIS-GRAPH-001 (P1). Regla de oro #97. Aprendizaje #157.
> - **Cross-refs:** Directrices v107.0.0, Arquitectura v96.0.0, Flujo v60.0.0.

> **🛡️ SISTEMA DE VALIDACION ARQUITECTONICA AUTOMATIZADA** (2026-03-03)
> - **Contexto:** 3 categorias de errores runtime (DI type mismatches, route-controller inexistentes, query chain bugs) no detectados por PHPStan/PHPCS/PHPUnit. Implementados 6 scripts de validacion cruzada YAML↔PHP con auto-descubrimiento y 4 puntos de control (pre-commit, CI, deploy, lando).
...
## 3.1 Case Studies — Storytelling de Conversión (CASE-STUDY-PATTERN-001)

📁 **Ubicación:** `docs/negocio/` (Word) + código en módulos custom + templates en tema
...
| [2026-03-10_integracion-firma-electronica-andalucia-ei.md](./tareas/2026-03-10_integracion-firma-electronica-andalucia-ei.md) | **Plan Maestro Andalucía +ei Clase Mundial — 12 Sprints: Firma electrónica (1-6) + Emprendimiento inclusivo (7-12)** ⭐ | 2026-03-10 |
...
|-------|---------|-------------|
| 2026-03-28 | **206.0.0** | **Mobile-First 10/10 Clase Mundial (VIEWPORT-DVH-001 + NOWRAP-OVERFLOW-001):** Auditoría exhaustiva frontend SaaS + 3 meta-sitios (score 7.2→10/10). 5 reglas nuevas: VIEWPORT-DVH-001 (P0, 40 dvh fallbacks en 18 SCSS), NOWRAP-OVERFLOW-001 (82/82 protegidas), FIXED-STACKING-MOBILE-001 (FABs conscientes bottom-nav), IMG-SEED-001 (idempotente deploy.yml), IMG-DIMENSIONS-001. 3 validators nuevos (viewport-dvh, nowrap-overflow, img-dimensions). Mobile menu dvh+padding responsive (landscape 500px). Setup Wizard ring 48px mobile + daily actions primary card diferenciada. Canvas editor redirect mobile. touch-action: pan-x en 4 scrollables. 15 conversiones font-size px→rem. SCSS-CSS-COSTAGE-001 fix falsos positivos parciales. settings.secrets.php SES vars. Docs: auditoría + plan implementación. Aprendizaje #236. |
...
| 2026-03-18 | **174.0.0** | **Freemium + Pagos Rápidos + PLG Configurables + Kit Digital Clase Mundial:** 8 planes Free €0 creados (1 por vertical). Promo banner sistema configurable desde Theme Settings (6 puntos entrada: header, footer, pricing, homepage, landings, checkout). Bizum botón checkout con Redsys callback processing. WhatsApp order verificado conectado. Hreflang triple-prefix fix (es/es/es → absoluto). Kit Digital _kit-digital.scss incluido en main.scss (557 LOC no cargaban). Logos oficiales institucionales descargados de kitdigital.red.es + ec.europa.eu (SVG Kit Digital, PNG NextGenerationEU ES, Ministerio, Red.es). Icono ai/rocket duotone creado (elimina emoji fallback 📌). 26/26 validaciones safeguard PASS. Regla de oro #136 (texto PLG requiere respaldo en BD). Aprendizaje #195. |
| 2026-03-18 | **172.0.0** | **Auditoría Integral SaaS + Plan 10/10 Clase Mundial — Sprint 1-6 completados (8.8→9.7/10):** Revisión profunda codebase (94 módulos, 1.131 servicios, 275+ entities). 7 market research docs (60+ competidores, 10 verticales). Auditoría Doc Consolidado v2 (7 errores, 18 correcciones). GP-06 Stripe Customer Portal (botón + JS fetch + library). GP-05 Usage warning banner (FairUsePolicyService warn/throttle en dashboards). AUDIT-CONS-001: entity_type_alter cubre 38 entities con DefaultEntityAccessControlHandler (1 línea). Schema.org SoftwareApplication por vertical (7 schemas). Lighthouse CWV audit job en CI. North Star metric widget. MR-01/05 MetaSitios corregidos (HomepageContent tenant_id + 2 TenantThemeConfig). Doc 158 v2 (pricing SSOT, 28 Stripe products, 4 bundles). CLAUDE.md +7 omisiones. 6 validaciones PASS. Regla de oro #134 (auditoría detecta que documentación subestima progreso: 3 features "Pendiente" ya implementadas). Aprendizaje #193. |
| 2026-03-16 | **163.0.0** | **Auditoría Integral + Setup Wizard 10/10 Clase Mundial — PIPELINE-E2E-001 + DailyActionsRegistry:** Nueva regla PIPELINE-E2E-001 (salvaguarda — toda implementación verifica 4 capas L1 Service/DI → L2 Controller → L3 hook_theme → L4 Template). DailyActionsRegistry infraestructura transversal (DailyActionInterface + DailyActionsRegistry + DailyActionsCompilerPass). 102 tagged services (48 wizard steps + 54 daily actions) en 11 módulos. 13 dashboards integrados (9 roles primarios + 4 secundarios: Orientador, Learner, Mentor, Entrepreneur). ACCESS-RETURN-TYPE-001: 68 access handlers corregidos (AccessResult → AccessResultInterface). FIELD-UI-SETTINGS-TAB-001: 12 rutas Field UI settings añadidas. CSS-VAR-ALL-COLORS-001: 6 SCSS tokenizados. VIEWS-DATA-001: 2 entities con views_data handler (A2ATask, ProactiveInsight). Aprendizaje #184. Regla de oro #125. Docs: `docs/analisis/2026-03-16_Auditoria_Integral_SaaS_*_v1.md` (v2.0), `docs/implementacion/2026-03-16_Plan_Implementacion_*_v1.md` (v2.0). |
| 2026-03-13 | **161.0.0** | **SETUP-WIZARD-DAILY-001 Patrón Transversal + Premium World-Class UI:** 2 reglas nuevas en DIRECTRICES v132: SETUP-WIZARD-DAILY-001 (P0, patron SaaS transversal — SetupWizardStepInterface 13 metodos + SetupWizardRegistry tagged services + CompilerPass + API REST async + parciales Twig + 10 keyframes CSS + JS celebrations), ICON-SYMLINK-REL-001 (P1, symlinks SVG relativos obligatorio para Docker/Lando). ARQUITECTURA v120: nueva seccion "SETUP WIZARD: PATRON TRANSVERSAL" + icon count actualizado ~366. Caso vertical Andalucia +ei: 4 wizard steps PIIL + 5 daily action cards + CoordinadorFormController slide-panel. 14 SVGs nuevos. Regla de oro #123 (symlinks en proyectos Docker siempre relativos). Aprendizaje #182 (patrón Setup Wizard tagged services — extensibilidad zero-touch via service tags). |
| 2026-03-13 | **160.0.0** | **ORTOGRAFIA-TRANS-001 Salvaguarda Ortográfica 3 Capas + Coordinador Hub Premium Elevation:** Nueva regla ORTOGRAFIA-TRANS-001 en DIRECTRICES v131 (3 capas: post-edit-lint + lint-staged + validate-all.sh con diccionario 402 entradas tildes+ñ + detección genérica -cion/-sion). ARQUITECTURA v119: seccion 10.8.1 actualizada a 19 scripts (+validate-twig-ortografia.php). FLUJO v84: learning #181 (salvaguarda ortográfica + premium UI patterns). Coordinador Hub: tabs "white pill on tinted rail" con borde 1.5px activo, compliance cards con --ring-color CSS cascade. 38+ errores ortográficos corregidos en coordinador-dashboard.html.twig. Regla de oro #122. Aprendizaje #181. |
| 2026-03-13 | **159.0.0** | **Premium Forms Slide-Panel UX — Checkbox Scoping + Observer Root + Sticky Wrapping Nav:** 3 reglas nuevas en DIRECTRICES v130: CHECKBOX-SCOPE-001 (P1, toggle switch scoped a `.form-type-boolean`, card-style para `.form-checkboxes`/`.form-radios`), OBSERVER-SCROLL-ROOT-001 (P1, IntersectionObserver root = `.slide-panel__body` en slide-panel), STICKY-NAV-WRAP-001 (P1, nav pills `flex-wrap: wrap` + sticky + backdrop-filter blur). ARQUITECTURA v118: seccion Premium Forms actualizada con 3 sub-reglas. FLUJO v83: learning #180 (3 patrones slide-panel UX). `_premium-forms.scss` + `premium-forms.js` modificados. Regla de oro #121. Aprendizaje #180. |
| 2026-03-11 | **154.0.0** | **ENV-PARITY-001 Dev/Prod Parity + 18 Scripts Validacion + Infraestructura Produccion:** Nuevo safeguard ENV-PARITY-001 (validate-env-parity.php ~900 LOC, 14 checks paridad dev/prod). config/deploy/mariadb/my.cnf (InnoDB 16G, NVMe I/O tuning). deploy.yml +OPcache invalidation. composer.json +12 ext-*. php.ini +max_input_vars 5000. validate-all.sh +ENV-PARITY-001 + 3 skip_checks. Analisis runbook AMD EPYC Zen 5 AE12-128 NVMe (docs/tecnicos/20260311b). Regla de oro #116. Aprendizaje #175. |
| 2026-03-10 | **151.0.0** | **Plan Maestro Andalucía +ei Clase Mundial — 12 Sprints completos:** Parte I Firma Electrónica (FirmaWorkflowService 8 estados, DocumentoFirmaOrchestrator 37 categorías, 7 API endpoints, firma táctil/AutoFirma/sello, QR verificación, TSA, firma masiva). Parte II Emprendimiento Inclusivo (PlanEmprendimientoEi entity 20 campos, 7 bridge services, matching Qdrant, alumni, badges, push notifications, copilot context). 10 gaps P0 resueltos: 2 permisos, 3 preprocess hooks, firma-masiva.js, 4 servicios cableados, 3 SCSS + libraries. 231 tests. Regla de oro #112. Aprendizaje #172. |
| 2026-03-10 | **152.0.0** | **Anti-Spam Stack Consolidation + Coordinador Dashboard Clase Mundial + Reclutamiento Landing SEO + 312 Unit Tests:** Anti-spam stack consolidation: reCAPTCHA v3 keys via deploy secrets (RECAPTCHA-ENV-INJECT-001), login/password solo reCAPTCHA + flood (ANTI-SPAM-STACK-001), antibot/honeypot eliminados de formularios login. Coordinador dashboard: 19 gaps en 3 sprints P0/P1/P2 — empty states siempre visibles (alertas, riesgo abandono), focus trap Tab/Shift+Tab en modales, CSRF token TTL 1h (CSRF-JS-CACHE-001), aria-live polite, Drupal.behaviors detach clearInterval, responsive tables sticky first column, prefers-reduced-motion, focus-visible. /user profile: widgets Andalucía +ei con badges por fase (5 variantes color-mix), stats row success/alert, completitud 100% success. Reclutamiento landing: social proof 4-card grid, lead magnet guía, 12 FAQs SEO People Also Ask, BreadcrumbList schema, og:image 1200x630. cross_vertical fix (solo TRUE para jaraba_mentoring). StoExportForm + download route. 21 test files nuevos → 312 tests / 1122 assertions / 0 errores. Reglas de oro #113-114. Aprendizaje #173. |
| 2026-03-09 | **150.0.0** | **Twig Render Array Safety + Entity Update Hook Hardening:** Descubrimiento critico: `url()` en Drupal 11 Twig devuelve render array (TwigExtension::getUrl() lineas 244-247), no string — concatenar con `~` genera "Array to string conversion". Fix _seo-schema.html.twig, orphan metatag field storage en billing_usage_record y site_config, 6 campos content_translation en site_config. 4 reglas: TWIG-URL-RENDER-ARRAY-001 (P0), TWIG-INCLUDE-ONLY-001 (P1), UPDATE-HOOK-FIELDABLE-001 (P0), TRANSLATABLE-FIELDS-INSTALL-001 (P0). Regla de oro #111. Aprendizaje #171. |
| 2026-03-06 | **149.0.0** | **SEO/GEO Meta-Sitio Corporativo 10/10 + Landing Reclutamiento Elevation:** og:image fallback cascade 5 niveles, hreflang dedup condicional, orphan templates resueltos, NAP normalizado, cookie localStorage-first, landing reclutamiento ZERO-REGION-003 + EducationalOccupationalProgram schema. 5 reglas: SEO-OG-IMAGE-FALLBACK-001, HREFLANG-CONDITIONAL-001, NAP-NORMALIZATION-001, ORPHAN-TEMPLATE-001, COOKIE-CONSENT-LOCAL-FIRST. Reglas de oro #106-108. Aprendizaje #170. |
| 2026-03-06 | **148.0.0** | **Cumplimiento PIIL CV 2025 — 6 fases canónicas + 3 entities STO + alertas normativas:** Modelo 6 fases (acogida→diagnóstico→atención→inserción→seguimiento→baja). ActuacionSto (22 campos, VoBo SAE), IndicadorFsePlus (26 campos, 3 momentos FSE+), InsercionLaboral (20+ campos condicionales). 6 servicios: DaciService, ReciboServicioService, ActuacionStoService, AlertasNormativasService, CalendarioProgramaService, FaseTransitionManager. 102 unit tests. Regla de oro #110. Aprendizaje #169. |
...
| 2026-03-04 | **139.0.0** | **CI/PHPStan Fixes — ACCESS-RETURN-TYPE-001 + PHPStan baseline 41K + CI pipeline verde:** PHPStan Level 6: 1890 errores corregidos en baseline (return type AccessResult→AccessResultInterface en 130+ AccessControlHandlers, entity field access). CI fitness-functions pipeline: 35→0 errores. Kernel tests: 460 tests, 2243 assertions, 0 errores. 1 regla nueva: ACCESS-RETURN-TYPE-001 (P0). Regla de oro #101. Aprendizaje #160: `docs/tecnicos/aprendizajes/2026-03-04_ci_phpstan_fixes_access_return_type.md`. |
| 2026-03-04 | **138.0.0** | **Stripe Connect Integration + DI Audit Runtime Fixes:** Stripe Connect configurado: tipo plataforma/marketplace, destination charges, subscriptions + metered billing, 2 webhook endpoints (billing /api/v1/billing/webhook + FOC /api/v1/foc/webhook) con secrets separados. settings.secrets.php ampliado: 3 namespaces config (ecosistema_jaraba_core.stripe, jaraba_foc.settings, jaraba_legal_billing.settings) via 4 env vars. StripeConnectService::getSecretKey() con getenv() fallback. Stripe CLI v1.37.2. DI audit: 21 mismatches corregidos en 9 modulos — 12 logger channel→factory (pixels 7, comercio_conecta 8, agroconecta 3, business_tools 4, resources 1), 6 phantom args eliminados (candidate 3, job_board 1, lms 3), 6 dependencias circulares resueltas (interactive YAML indent, core style_preset 4→2, foc metrics_calculator, email campaign_service, lms course+certification+xapi, resources subscription_service). PHP 8.4 nullable fix FlashOfferService (?float). 4 reglas: LOGGER-DI-001, CIRCULAR-DEP-001, PHANTOM-ARG-001, STRIPE-ENV-UNIFY-001. 3 gaps salvaguarda identificados. Reglas de oro #99-100. Aprendizaje #159: `docs/tecnicos/aprendizajes/2026-03-04_stripe_integration_di_audit_runtime_fixes.md`. |
| 2026-03-04 | **137.0.0** | **7 Items SaaS Post-Evaluacion — VerticalBrand + Pre-PMF + PilotManager + PIIL + OpenAPI + Safeguards:** Implementacion de 7 items estrategicos post-evaluacion. Item 1 VerticalBrand ConfigEntity (18 campos, 10 YAMLs) + 3 Services. Item 2 Pre-PMF analytics (ProductMetricSnapshot + ActivationCriteriaConfig + 5 Services). Item 3 Twig feature_allowed (FeatureGateRouterService ServiceLocator). Item 4 PilotManager modulo nuevo (3 entities + services + ECA). Item 5 PIIL extension (+13 campos + 2 services). Item 6 OpenAPI (spec dinamica). Item 7 Safeguards (CHECK 7 + UPDATE-HOOK-REQUIRED-001 ConfigEntities). 40 tests, 294 assertions. Regla de oro #98. Aprendizaje #158. |
| 2026-03-04 | **136.0.0** | **LCIS Gap Closure — 2 Entities + 2 QueueWorkers + EU AI Act Audit Trail:** 4 gaps de complitud cerrados entre spec 179 y la implementacion real del LCIS. G1: `legal_norm_relation` entity (jaraba_legal_knowledge, 10 tipos relacion normativa, grafo de derogaciones/modificaciones para NormativeGraphEnricher L3). G2: `legal_coherence_log` entity (jaraba_legal_intelligence, 15 campos audit trail EU AI Act Art. 12, read-only, trace_id vinculado a TraceContextService). G3: `NormativeUpdateWorker` QueueWorker (cron 60s, actualiza vigencia normas + crea relaciones). G4: `NormVectorizationWorker` QueueWorker (cron 120s, chunking + embeddings + Qdrant upsert, defensivo). 3 servicios LCIS modificados: NormativeGraphEnricher (+graph queries reales), LegalCoherenceValidatorService (+audit log), LegalCoherenceVerifierService (+audit log). 5 tests nuevos (2 Unit MOCK-DYNPROP-001 + 3 Kernel con synthetic services). 10 ficheros nuevos, 9 modificados. 2 hook_update_N(). 2 reglas: LCIS-AUDIT-001 (P0), LCIS-GRAPH-001 (P1). Regla de oro #97. Aprendizaje #157: `docs/tecnicos/aprendizajes/2026-03-04_lcis_gap_closure_entities_queueworkers.md`. |
| 2026-03-03 | **135.0.0** | **Sistema de Validacion Arquitectonica Automatizada:** 6 scripts de validacion cruzada YAML↔PHP en `scripts/validation/` con auto-descubrimiento dinamico. Orquestador --fast (pre-commit <3s) y --full (CI/deploy). 4 puntos de integracion (pre-commit, ci.yml, deploy.yml, lando). 6 violaciones pre-existentes corregidas. 4 bugs runtime corregidos (3 ECA event_name + 1 $wContract). 3 reglas: ARCH-VALIDATE-001, ECA-EVENT-001, BASELINE-CLEAN-001. Reglas de oro #94-96. Aprendizaje #156: `docs/tecnicos/aprendizajes/2026-03-03_validacion_arquitectonica_sistema_automatizado.md`. |
| 2026-02-28 | **133.0.0** | **Claude Code Development Pipeline — DX Clase Mundial con 14 Ficheros + 30 Tests Funcionales:** Pipeline completo de desarrollo asistido por IA implementado en `.claude/` con 5 capas: (1) Proteccion PreToolUse (`pre-tool-use.sh` bloquea 7 archivos criticos, advierte master docs), (2) Lint PostToolUse (`post-edit-lint.sh` valida PHP/SCSS/JS/Twig contra directrices), (3) Subagentes (reviewer con 20+ directrices, tester con 7 reglas PHP 8.4, security-auditor con 10 categorias), (4) Slash Commands (/fix-issue workflow 8 pasos, /create-entity scaffold 9 archivos, /verify-runtime 12 checks, /audit-wcag 10 categorias WCAG 2.1 AA), (5) Integraciones (MCP Stripe, CLAUDE.md 236 LOC, Estado SaaS 793 LOC). settings.json con hooks config PreToolUse+PostToolUse. 30 tests funcionales: 5 pre-tool-use (deny/ask/allow con exit codes), 14 post-edit-lint (4 tipos archivo × escenarios), 5 verify-runtime (checks contra jaraba_support), 5 audit-wcag (checks contra templates reales), 1 MCP Stripe (npx disponible). 3 bugs corregidos: Python extraction AttributeError, output channel stderr→stdout, multiline grep false positives. Documentos: `docs/implementacion/2026-02-28_Plan_Implementacion_Claude_Code_Pipeline_v1.md` (742 LOC), `docs/tecnicos/2026-02-28_Estado_SaaS_Claude_Chat_v1.md` (793 LOC). Arquitectura v93.0.0 (seccion 10.6 ASCII box). 3 reglas: HOOK-PRETOOLUSE-001, HOOK-POSTLINT-001, CLAUDE-SUBAGENT-001. Regla de oro #91. Aprendizaje #154. |
