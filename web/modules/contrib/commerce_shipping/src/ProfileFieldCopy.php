...
class ProfileFieldCopy implements ProfileFieldCopyInterface {
  ...
  public function alterForm(array &$inline_form, FormStateInterface $form_state) {
    ...
    // When in the admin, skip presenting the profile copy checkbox if the
    // shipping profile is new.
    // A new empty shipping profile is initialized by the
    // ShippingProfileSubscriber, but in the admin, the shipping information is
    // not entered within the same form / page as opposed to "checkout" where we
    // should still allow copying the profile.
    ...
    // When the selected shipping country is not supported in the store's
    // billing county list, the "Billing same as shipping" option is hidden
    // and all billing address fields are displayed in address form.
    if (!$this->isShippingCountrySupportedForBilling($form_state)) {
      $inline_form['copy_fields']['enable']['#access'] = FALSE;
    ...
    if ($enabled) {
      ...
      // Render the shipping profile about to be copied in the admin.
      if (!empty($configuration['admin'])) {
        $view_builder = $this->entityTypeManager->getViewBuilder('profile');
        $inline_form['rendered_shipping_profile'] = [
          '#type' => 'fieldset',
          '#title' => $this->t('Current shipping information'),
          '#title_display' => 'before',
          'content' => $view_builder->view($shipping_profile),
        ];
      }
  ...
  public static function validateForm(array &$inline_form, FormStateInterface $form_state) {
    ...
    // Validate billing country when reusing shipping address.
    if ($form_state->getValue($inline_form['copy_fields']['enable']['#parents'])
      && !self::isShippingCountrySupportedForBilling($form_state)
    ) {
      $form_state->setError($inline_form['copy_fields']['enable'], new TranslatableMarkup('Shipping Address country is not a valid billing country. Please enter a different billing address.'));
    }
  }
  ...
  /**
   * Checks whether the shipping address can be used for billing.
   *
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The current state of the form.
   *
   * @return bool
   *   Whether the shipping address can be used for billing.
   */
  protected static function isShippingCountrySupportedForBilling(FormStateInterface $form_state): bool {
    // Return TRUE when no billing countries are provided.
    $store = static::getOrder($form_state)?->getStore();
    if (empty($store?->getBillingCountries())) {
      return TRUE;
    }

    // Return TRUE when shipping does not have a country.
    $shipping_profile = static::getShippingProfile($form_state);
    /** @var \Drupal\address\Plugin\Field\FieldType\AddressItem|null $shipping_address */
    $shipping_address = $shipping_profile->get('address')->first();
    if (empty($shipping_address?->getCountryCode())) {
      return TRUE;
    }

    return in_array($shipping_address->getCountryCode(), $store->getBillingCountries(), TRUE);
  }
