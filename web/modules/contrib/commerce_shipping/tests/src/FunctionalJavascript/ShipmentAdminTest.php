...
class ShipmentAdminTest extends CommerceWebDriverTestBase {
  ...
  public function testAddShippingInModal(): void {
    ...
    $this->assertRenderedAddressInRegion($this->defaultAddress, $shipping_information_card);
    /** @var \Drupal\commerce_shipping\Entity\ShipmentInterface $shipment */
    $shipment = $this->order->get('shipments')->entity;
    $this->assertTrue($shipment->getData('owned_by_packer'));
    $card_text = $shipping_information_card->getText();
    $this->assertStringContainsString('Shipping method ' . $shipment->getShippingMethod()->label(), $card_text);
    $this->assertStringContainsString('Amount ' . $shipment->get('amount')->formatted, $card_text);
    $this->assertStringContainsString('Tracking code ' . $shipment->getTrackingCode(), $card_text);
  }
  ...
  public function testEditShipmentInModal(): void {
    ...
    $shipping_information_card = $this->getShippingInformationAdminCard();
    $this->assertRenderedAddressInRegion($this->defaultAddress, $shipping_information_card);
    $this->assertStringContainsString('Shipping method Overnight shipping', $shipping_information_card->getText());
    $this->assertStringContainsString('Amount $19.99', $shipping_information_card->getText());
    ...
    $this->assertRenderedAddressInRegion($this->defaultAddress, $shipping_information_card);
    $card_text = $shipping_information_card->getText();
    $this->assertStringContainsString('Shipping method Standard shipping', $card_text);
    $this->assertStringContainsString('Amount $9.99', $card_text);
    $this->assertStringContainsString('Tracking code A1234567890', $card_text);
  ...
  /**
   * Test draft shipment is recalculated for empty orders.
   */
  public function testDraftShipmentRepack() {
    // Create draft order.
    ...
    // Update shipping methods condition to skip orders with zero total.
    $conditions = [
      [
    ...
    $shipping_method_storage = $this->container->get('entity_type.manager')
      ->getStorage('commerce_shipping_method');
    foreach ($shipping_method_storage->loadMultiple() as $shipping_method) {
      $shipping_method->set('conditions', $conditions);
      $shipping_method->save();
    }

    // Create draft shipment without any items.
    ...
    $this->assertTrue($shipment->getData('owned_by_packer'));
    $this->assertNull($shipment->getShippingMethod());
    $this->assertEmpty($shipment->getItems());

    // Add items back to the order to confirm that shipment items and method are
    // added to the related shipment.
    foreach ($this->order->getItems() as $item) {
    ...
    /** @var \Drupal\commerce_shipping\Entity\ShipmentInterface $shipment */
    $shipment = $this->reloadEntity($shipment);
    $this->assertNotEmpty($shipment->getItems());
    $this->assertInstanceOf(ShippingMethodInterface::class, $shipment->getShippingMethod());

    // Check that shipping information is shown on the order view page.
    $this->drupalGet($order->toUrl());
    $card_text = $this->getShippingInformationAdminCard()->getText();
    $this->assertStringContainsString('Shipping method ' . $shipment->getShippingMethod()->label(), $card_text);
    $this->assertStringContainsString('Amount ' . $shipment->get('amount')->formatted, $card_text);
  }

  /**
   * Returns Shipping information card.
   */
  protected function getShippingInformationAdminCard() {
    return $this->getSession()->getPage()->find('css', '#shipping-information-admin-card');
  }
