...
namespace Drupal\jaraba_comercio_conecta\Grounding;
...
/**
 * Grounding provider para productos retail (product_retail).
 */
class ProductRetailGroundingProvider implements GroundingProviderInterface {
  ...
  public function search(array $keywords, int $limit = 3): array {
    ...
    foreach ($entities as $entity) {
      ...
      $results[] = [
        'title' => $entity->label() ?? 'Producto',
        'summary' => mb_substr(strip_tags((string) ($entity->get('short_description')->value ?? $entity->get('description')->value ?? '')), 0, 200),
        'url' => $entity->toUrl()->toString(),
        'type' => 'Producto — ComercioConecta',
        'metadata' => [
          'precio' => $price,
        ],
      ];
