...
/**
 * Verifica que el usuario es el propietario (initiated_by) de la conversación.
 *
 * LÓGICA:
 * - Carga SecureConversation por UUID del route parameter {uuid}.
 * - Compara initiated_by con el ID del usuario actual.
 * - Permite acceso si coincide o si tiene 'administer jaraba messaging'.
 */
class ConversationOwnerAccessCheck implements AccessInterface {
  ...
  /**
   * Checks access for conversation owner routes.
   *
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account): AccessResultInterface {
    ...
    // Verificar que el usuario es quien inició la conversación.
    ...
    return AccessResult::forbidden('User is not the owner of this conversation.')
      ->addCacheableDependency($conversation)
      ->cachePerUser();
  }
