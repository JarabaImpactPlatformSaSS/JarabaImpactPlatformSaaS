...
abstract class SmartBaseAgent extends BaseAgent {
  ...
  protected function callAiApi(string $prompt, array $options = []): array {
    ...
    try {
      ...
      if (LegalCoherencePromptRule::requiresCoherence(
          $options['action'] ?? '',
          $this->vertical ?? '',
        )) {
        $short = LegalCoherencePromptRule::useShortVersion($options['action'] ?? '');
        $systemPrompt = LegalCoherencePromptRule::apply($systemPrompt, $short);
      }
