...
class CopilotOrchestratorService {
  ...
  // Gemini primario (40% trafico, mas barato)
    'consultor' => ['google_gemini', 'anthropic', 'openai'],
    'sparring' => ['anthropic', 'openai', 'google_gemini'],
  // GPT-4 mejor en calculos.
    'cfo' => ['openai', 'anthropic', 'google_gemini'],
  // Claude principal (RAG)
    'fiscal' => ['anthropic', 'google_gemini'],
  // Claude principal (RAG)
    'laboral' => ['anthropic', 'google_gemini'],
    'devil' => ['anthropic', 'openai', 'google_gemini'],
  // Gemini primario (landing=alto volumen)
  ...
  // Economico para deteccion.
    'detection' => 'claude-haiku-4-5-20251001',
  // Gemini Flash (alto volumen)
    'landing_copilot' => 'gemini-2.5-flash',
  ...
  protected function buildSystemPrompt(array $context, string $mode): string {
    ...
    $identityRule = AIIdentityRule::IDENTITY_PROMPT;
    ...
    if (in_array($mode, ['fiscal', 'laboral'], TRUE)) {
      $legalCoherencePrompt = LegalCoherencePromptRule::COHERENCE_PROMPT_SHORT;
    }
