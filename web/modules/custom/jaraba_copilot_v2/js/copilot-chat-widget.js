...
(function (Drupal, once) {
  ...
  function initChatWidget(el) {
    ...
    function renderMessages() {
      ...
      state.messages.forEach(function (msg, idx) {
        if (msg.role === 'user') {
          ...
          html += '<div class="copilot-chat__message-content">' + parseMarkdown(msg.text) + '</div>';
      ...
      if (state.currentResponse && !state.thinking) {
        ...
        html += '<div class="copilot-chat__message-content">' + parseMarkdown(state.currentResponse);
     ...
     * Orden importa: bold+italic primero para evitar conflictos.
     */
     ...
     * Devuelve HTML string o null si no es tabla válida.
     */
     ...
     *
     * Soporta: headers, bold/italic/strikethrough, code inline y bloques,
     * listas OL/UL, blockquotes, tablas, separadores, enlaces, CTAs.
     */
    function parseMarkdown(text) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        // Code block placeholder (ya procesado).
        if (line.trim().indexOf('__CODEBLOCK_') === 0 || line.trim().indexOf('<div class="copilot-codeblock"') === 0) {
          closeLists(); closeBlockquote();
          result.push(line.trim());
          continue;
        }

        // Tabla: detectar header + separator en lineas i, i+1.
        ...
        // Lista desordenada: - item o * item (solo * con espacio para evitar conflicto con bold)
