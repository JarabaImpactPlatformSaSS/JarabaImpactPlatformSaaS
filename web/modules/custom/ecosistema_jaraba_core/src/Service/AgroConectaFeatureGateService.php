 ...
 * y el contador de uso en la tabla {agroconecta_feature_usage}.
 * Integra con UpgradeTriggerService para disparar modales de upgrade.
 *
 * Plan Elevacion AgroConecta Clase Mundial v1 — Fase 0
 *
 * Features gestionadas:
 * - products: Productos publicados (5/25/-1)
 * - orders_per_month: Pedidos mensuales recibidos (10/50/-1)
 * - copilot_uses_per_month: Usos mensuales del copilot IA (5/50/-1)
 * - photos_per_product: Fotos por producto (3/10/-1)
 * - commission_pct: Porcentaje de comision (15/10/5)
 * - traceability_qr: QR de trazabilidad (0/5/-1)
 * - partner_hub: Acceso Hub B2B (0/0/-1)
 * - analytics_advanced: Analytics avanzados (0/0/-1)
 * - demand_forecaster: Prediccion de demanda IA (0/0/-1)
 *
 ...
 */
class AgroConectaFeatureGateService {
  ...
  /**
   * Verifica si el usuario puede usar una feature del vertical AgroConecta.
   *
   * @param int $userId
   *   El ID del usuario.
   * @param string $featureKey
   *   La feature a verificar (products, orders_per_month, etc.).
  ...
  public function recordUsage(int $userId, string $featureKey): void {
    ...
    $existing = $this->database->select('agroconecta_feature_usage', 'u')
    ...
    if ($existing) {
      $this->database->update('agroconecta_feature_usage')
        ->fields(['usage_count' => $existing->usage_count + 1])
        ->condition('id', $existing->id)
        ->execute();
    }
    else {
      $this->database->insert('agroconecta_feature_usage')
        ->fields([
          'user_id' => $userId,
    ...
    $this->logger->info('AgroConecta feature usage recorded: @feature for user @user (period: @period)', [
      '@feature' => $featureKey,
      '@user' => $userId,
      '@period' => $period,
    ]);
  ...
  /**
   * Obtiene el plan actual del usuario para el vertical AgroConecta.
   *
  ...
  public function resetMonthlyUsage(): void {
    ...
    $deleted = $this->database->delete('agroconecta_feature_usage')
      ->condition('usage_date', $previousMonth . '%', 'LIKE')
      ->execute();

    $this->logger->info('AgroConecta monthly usage reset: @count records deleted for period @period', [
      '@count' => $deleted,
  ...
  /**
   * Obtiene el porcentaje de comision para un productor segun su plan.
   *
   * @param int $userId
   *   El ID del productor.
   *
  ...
  protected function getUsageCount(int $userId, string $featureKey): int {
    ...
    $count = $this->database->select('agroconecta_feature_usage', 'u')
   ...
   *
   * Para 'products': cuenta ProductAgro activos del usuario.
   * Para 'photos_per_product': cuenta fotos del producto especifico.
   *
   * @param int $userId
   *   El ID del usuario.
   * @param string $featureKey
  ...
  protected function getDefaultUpgradeMessage(string $featureKey): string {
    $messages = [
      ...
      'orders_per_month' => 'Has alcanzado el limite de pedidos mensuales. Actualiza para no perder ventas.',
      'copilot_uses_per_month' => 'Has agotado tus consultas IA del mes. Actualiza para mas usos.',
      'photos_per_product' => 'Has alcanzado el limite de fotos por producto. Actualiza para subir mas.',
      'traceability_qr' => 'La trazabilidad QR no esta disponible en tu plan actual.',
      'partner_hub' => 'El Hub B2B no esta disponible en tu plan actual.',
      'analytics_advanced' => 'Los analytics avanzados no estan disponibles en tu plan actual.',
      'demand_forecaster' => 'La prediccion de demanda IA no esta disponible en tu plan actual.',
    ];
  ...
  protected function ensureTable(): void {
    if (!$this->database->schema()->tableExists('agroconecta_feature_usage')) {
      $this->database->schema()->createTable('agroconecta_feature_usage', [
