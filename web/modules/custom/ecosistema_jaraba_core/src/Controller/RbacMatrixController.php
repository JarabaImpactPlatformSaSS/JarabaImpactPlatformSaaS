...
class RbacMatrixController extends ControllerBase {
  ...
  /**
   * Constructs a RbacMatrixController.
   */
  ...
  public function matrix(): array {
    ...
    $toggleUrl = Url::fromRoute('ecosistema_jaraba_core.api.rbac.toggle')
      ->toString();
  ...
  public function togglePermission(Request $request): JsonResponse {
    ...
    if (empty($data['role']) || empty($data['permission'])) {
      return new JsonResponse([
        'success' => FALSE,
        'error' => $this->t('Los campos rol y permiso son obligatorios.'),
      ], 400);
    }
