 ...
 * @coversDefaultClass \Drupal\ecosistema_jaraba_core\Service\AuditLogService
 */
class AuditLogServiceTest extends TestCase {
  ...
  /**
   * Tests that log() creates an audit log entity with correct field values.
   *
   * @covers ::log
   */
  public function testLogCreatesAuditLogEntity(): void {
    ...
    $this->storage->expects($this->once())
      ...
      ->with($this->callback(function (array $values): bool {
        return $values['event_type'] === 'user_login'
          && $values['actor_id'] === 42
    ...
    $this->service->log('user_login', ['severity' => 'info']);
  }
  ...
  public function testLogCatchesExceptions(): void {
    ...
    $this->logger->expects($this->once())
      ->method('error')
      ->with(
        'Failed to write audit log for event @event: @message',
        $this->callback(function (array $context): bool {
          return $context['@event'] === 'user_login'
  ...
  /**
   * Tests that log() handles details array in context.
   *
   * @covers ::log
   */
