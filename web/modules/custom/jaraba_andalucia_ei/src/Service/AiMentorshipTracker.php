...
class AiMentorshipTracker {
  ...
  /**
   * Gets today's IA mentorship hours for a participant entity.
   *
   * Public facade called from jaraba_andalucia_ei.module.
   *
   * @param \Drupal\jaraba_andalucia_ei\Entity\ProgramaParticipanteEiInterface $participante
   *   The participant entity.
   *
   * @return float
   *   Hours registered today.
   */
  public function getHoursToday(ProgramaParticipanteEiInterface $participante): float {
    return $this->getHorasIaHoy((int) $participante->id());
  }

  /**
   * Records a mentorship session in the audit table.
   *
   * Public facade called from jaraba_andalucia_ei.module.
   *
   * @param \Drupal\jaraba_andalucia_ei\Entity\ProgramaParticipanteEiInterface $participante
   ...
   *   Copilot session ID (stored as tipo_interaccion context).
   */
  public function recordSession(ProgramaParticipanteEiInterface $participante, float $horas, string|int $session_id): void {
    $this->registrarEnAuditoria(
      (int) $participante->id(),
      $horas,
      'copilot_session_' . $session_id,
    );
  }
  ...
  protected function registrarEnAuditoria(int $participante_id, float $horas, string $tipo): void {
    $this->database->insert('jaraba_andalucia_ei_ia_audit')
      ->fields([
        ...
        'horas' => $horas,
        'tipo_interaccion' => substr($tipo, 0, 32),
        'fecha' => date('Y-m-d'),
