...
/**
 * Access control handler for InscripcionSesionEi entities.
 *
 ...
 *
 * Participants can view/edit their own inscriptions (uid match).
 */
class InscripcionSesionEiAccessControlHandler extends EntityAccessControlHandler implements EntityHandlerInterface {
  ...
  protected function checkAccess(EntityInterface $entity, $operation, AccountInterface $account): AccessResultInterface {
    ...
    // Participants can view/edit their own inscriptions.
    $isOwner = $entity->hasField('uid')
      && (int) $entity->getOwnerId() === (int) $account->id();

    return match ($operation) {
      'view' => AccessResult::allowedIf($isOwner)
        ->orIf(AccessResult::allowedIfHasPermission($account, 'view inscripcion sesion ei'))
        ->cachePerUser()
        ->addCacheableDependency($entity),
      'update' => AccessResult::allowedIf($isOwner)
        ->orIf(AccessResult::allowedIfHasPermission($account, 'edit inscripcion sesion ei'))
        ->cachePerUser()
        ->addCacheableDependency($entity),
      'delete' => AccessResult::allowedIfHasPermission($account, 'delete inscripcion sesion ei')
        ->addCacheableDependency($entity),
